...
def test_ocr_cache_loads_results_and_updates_modification_time(tmp_path: Path):
    ...
    result = cache.load(image, cache_identity)
...
def test_ocr_cache_discards_mismatched_version(tmp_path: Path):
    ...
    assert cache.load(image, cache_identity) is None
...
def test_ocr_cache_overwrites_matching_result_once(tmp_path: Path):
    ...
    assert overwrite_cache.load(image, cache_identity) is None
    overwrite_cache.save(image, cache_identity, "fresh text")

    assert overwrite_cache.load(image, cache_identity) == "fresh text"
...
def test_ocr_cache_uses_runtime_default(runtime_cache_root_path: Path):
    ...
    assert cache.load(image, cache_identity) == "cached text"
