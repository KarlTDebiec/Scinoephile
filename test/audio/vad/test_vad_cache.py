...
def test_vad_cache_round_trip(tmp_path: Path):
    ...
    cache_path = cache.save(audio, cache_identity, trace)
...
def test_vad_cache_overwrites_each_entry_once(tmp_path: Path):
    ...
    VoiceActivityCache(tmp_path).save(audio, cache_identity, first_trace)
    overwrite_cache = VoiceActivityCache(tmp_path, overwrite=True)

    assert overwrite_cache.load(audio, cache_identity) is None
    overwrite_cache.save(audio, cache_identity, second_trace)
    loaded = overwrite_cache.load(audio, cache_identity)
...
def test_vad_cache_atomic_write_failure_preserves_existing_entry(
    ...
    cache.save(audio, cache_identity, trace)
