...
"""Tests of scinoephile.cli.zho.zho_translate_vs_eng_cli."""
...
def test_zho_translate_vs_eng_cli_gapped_translation():
    ...
    with get_temp_file_path(".srt") as output_path:
        with patch(
            "scinoephile.cli.zho.zho_translate_vs_eng_cli."
    ...
    assert (
        ...
        is ZhoGappedTranslationVsEngPromptZhoHans
    )
...
def test_zho_translate_vs_eng_cli_guided_translation():
    ...
    assert (
        patched_factory.call_args.kwargs["prompt_cls"]
        is ZhoGuidedTranslationVsEngPromptZhoHans
    )
    assert patched_factory.call_args.kwargs["provider"] is not None
    called_kwargs = patched_translate.call_args.kwargs
    assert_series_equal(called_kwargs["eng"], Series.load(eng_input_path))
...
def test_zho_translate_vs_eng_cli_regular_translation():
    ...
    with get_temp_file_path(".srt") as output_path:
        with patch(
            "scinoephile.cli.zho.zho_translate_vs_eng_cli.get_zho_eng_translator",
            return_value="translator",
        ) as patched_factory:
            with patch(
                "scinoephile.cli.zho.zho_translate_vs_eng_cli."
    ...
    assert (
        patched_factory.call_args.kwargs["prompt_cls"]
        is ZhoTranslationVsEngPromptZhoHans
    )
    assert patched_factory.call_args.kwargs["provider"] is not None
    called_kwargs = patched_translate.call_args.kwargs
    assert_series_equal(called_kwargs["eng"], Series.load(eng_input_path))
