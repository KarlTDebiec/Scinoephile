...
def test_zho_cli_legacy_under_scinoephile(
    input_path: str,
    args: str,
    expected_path: str,
):
    """Test legacy `scinoephile zho --...` behavior with optional subcommands.
    ...
    with get_temp_file_path(".srt") as output_path:
        run_cli_with_args(
            ScinoephileCli,
            f"zho --infile {full_input_path} {args} --outfile {output_path}",
        )
        output = Series.load(output_path)
        expected = Series.load(full_expected_path)
