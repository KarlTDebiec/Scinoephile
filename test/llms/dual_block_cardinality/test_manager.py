...
"""Tests for dual block cardinality LLM model factories."""
...
class _Prompt(DualBlockCardinalityPrompt):
    """Prompt fixture for dual block cardinality tests."""
...
def test_get_query_cls_rejects_empty_source_one():
    ...
    with pytest.raises(ScinoephileError):
        DualBlockCardinalityManager.get_query_cls(0, 1, _Prompt)


def test_get_query_cls_rejects_negative_source_two():
    """Test source two size may not be negative."""
    with pytest.raises(ScinoephileError):
        DualBlockCardinalityManager.get_query_cls(1, -1, _Prompt)
