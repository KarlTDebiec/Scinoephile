...
class _FakeAsyncChatApi:
    """Fake async chat API root with completions surface."""
...
class _FakeAsyncClient:
    """Fake async OpenAI client with chat API."""
...
def _get_tool_spec() -> LLMToolSpec:
    """Get a generic lookup tool spec for tests.

    Returns:
        generic lookup tool specification
    """
    return {
...
async def test_chat_completion_async_runs_tool_and_returns_final_content():
    ...
    content = await provider.chat_completion_async(
        messages=[
            {"role": "system", "content": "system"},
            {"role": "user", "content": "user"},
        ],
