...
def test_extract_audio_track_supports_native_center_heavy_mix(tmp_path: Path):
    """Test center-heavy extraction preserves rate and uses expected weights.
...
def test_extract_audio_track_supports_native_complete_mix(tmp_path: Path):
    """Test complete-stream extraction preserves the source sample rate.
    ...
    with (
        ...
        extract_audio(
            infile_path, tmp_path / "audio.wav", mode=AudioExtractionMode.NATIVE_STEREO
        )

    assert fake_ffmpeg_input.output_kwargs is not None
    assert fake_ffmpeg_input.output_kwargs["map"] == "0:12"
    assert fake_ffmpeg_input.output_kwargs["ac"] == 2
    assert "ar" not in fake_ffmpeg_input.output_kwargs


def test_extract_audio_track_rejects_center_mix_without_center(tmp_path: Path):
    """Test center extraction rejects streams without a center channel.
    ...
    with (
        ...
        raises(ScinoephileError, match="requires a stream with a center channel"),
    ):
        extract_audio(
