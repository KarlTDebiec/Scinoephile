#  Copyright 2017-2025 Karl T Debiec. All rights reserved. This software may be modified
#  and distributed under the terms of the BSD license. See the LICENSE file for details.
"""MLAMD 粤文 merging test cases."""

from __future__ import annotations

from itertools import chain

from scinoephile.audio.cantonese.merging import MergingTestCase

# noinspection PyArgumentList
merging_test_cases_block_0 = [
MergingTestCase(
    zhongwen="沿荔枝角道直出大角咀道",
    yuewen_to_merge=['沿住荔枝角度', '直出大角咀度'],
    yuewen_merged="沿住荔枝角度直出大角咀度",
    verified=True,
),
MergingTestCase(
    zhongwen="经好彩酒家左转花园街乐园牛丸王⋯",
    yuewen_to_merge=['经过好彩走家', '再左转返出花园街', '乐园牛园望对上'],
    yuewen_merged="经过好彩走家再左转返出花园街乐园牛园望对上⋯",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="更正一下：",
    yuewen_to_merge=['都系唔好'],
    yuewen_merged="都系唔好：",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="转呀，转⋯再更正一下：",
    yuewen_to_merge=['转下', '转下', '都系唔好'],
    yuewen_merged="转下，转下⋯都系唔好：",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="直出亚皆老街跨过火车桥右转太平道",
    yuewen_to_merge=['都系出返去阿街路街飞过火车桥', '右转入太平道'],
    yuewen_merged="都系出返去阿街路街飞过火车桥右转入太平道",
    verified=True,
),
MergingTestCase(
    zhongwen="再右拐窝打老道向女人街方向飞⋯",
    yuewen_to_merge=['再右转抹返出去窝打炉道', '向女人街方向飞下下'],
    yuewen_merged="再右转抹返出去窝打炉道向女人街方向飞下下⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="飞呀，飞⋯",
    yuewen_to_merge=['飞下', '飞下'],
    yuewen_merged="飞下，飞下⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="也就是在麦太右边额角上⋯",
    yuewen_to_merge=['亦即系麦太右边云晶对上'],
    yuewen_merged="亦即系麦太右边云晶对上⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="更正：左边额角上⋯",
    yuewen_to_merge=['都系唔好', '左边云晶对上'],
    yuewen_merged="都系唔好：左边云晶对上⋯",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="转呀，转⋯",
    yuewen_to_merge=['转下', '转下', '转下噉'],
    yuewen_merged="转下，转下，转下噉⋯",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="脑海中同时出现即将诞生的儿子容貌⋯",
    yuewen_to_merge=['而脑入面亦即时出现咗快要出世个仔嘅样'],
    yuewen_merged="而脑入面亦即时出现咗快要出世个仔嘅样⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="希望他好聪明，读书好叻！",
    yuewen_to_merge=['希望佢好聪明', '读书好叻'],
    yuewen_merged="希望佢好聪明，读书好叻！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="于是她向胶兜补充说：",
    yuewen_to_merge=['于是佢对住胶兜补充噉话'],
    yuewen_merged="于是佢对住胶兜补充噉话：",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="或者读书唔叻，工作叻呢？",
    yuewen_to_merge=['或者读书唔叻', '出嚟做嘢叻啦'],
    yuewen_merged="或者读书唔叻，出嚟做嘢叻啦？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="又或者⋯",
    yuewen_to_merge=['又或者呢'],
    yuewen_merged="又或者呢⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="又或者好靓仔，好靓仔",
    yuewen_to_merge=['又或者系好靓仔好靓仔'],
    yuewen_merged="又或者系好靓仔，好靓仔",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="跟周润发，梁朝伟那么靓仔！",
    yuewen_to_merge=['好似周润发同埋梁朝伟咁靓仔'],
    yuewen_merged="好似周润发，同埋梁朝伟咁靓仔！",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="胶兜仍然在转，毫无点头迹象",
    yuewen_to_merge=['胶兜依然系噉喺度转', '好似一啲应承嘅迹象都冇'],
    yuewen_merged="胶兜依然系噉喺度转，好似一啲应承嘅迹象都冇",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="赶忙趁胶兜落地前另许一个愿望：",
    yuewen_to_merge=['嗱嗱嗱喺胶兜未落地之前起过另外一个愿望'],
    yuewen_merged="嗱嗱嗱喺胶兜未落地之前起过另外一个愿望：",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="唔聪明唔靓仔也算了，只要福星高照",
    yuewen_to_merge=['就算唔系咁聪明同咁靓仔', '只要复星高照'],
    yuewen_merged="就算唔系咁聪明同咁靓仔，只要复星高照",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="一世够运，逢凶化吉！",
    yuewen_to_merge=['一世救运', '乜嘢事都逢凶化㗎喇'],
    yuewen_merged="一世救运，乜嘢事都逢凶化㗎喇！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="但总得要叻仔呀！",
    yuewen_to_merge=['但系都要叻仔先得㗎'],
    yuewen_merged="但系都要叻仔先得㗎！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_0
# noinspection PyArgumentList
merging_test_cases_block_1 = [
MergingTestCase(
    zhongwen="最后，胶兜「嘀督」一声落地",
    yuewen_to_merge=['最后胶兜滴嘟一声咁落地'],
    yuewen_merged="最后，胶兜「滴嘟」一声咁落地",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="嘀督？嘀督，就是答应了",
    yuewen_to_merge=['滴嘟', '滴嘟㖞', '即系应承啦'],
    yuewen_merged="滴嘟？滴嘟㖞，即系应承啦",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="麦太想，这次走运了！",
    yuewen_to_merge=['麦太心谂', '今次冇死喇'],
    yuewen_merged="麦太心谂，今次冇死喇！",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="可是答应了些什么呢？",
    yuewen_to_merge=['但你应承咗啲咩呢'],
    yuewen_merged="但你应承咗啲咩呢？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="叻仔？好运？",
    yuewen_to_merge=['叻仔', '好运'],
    yuewen_merged="叻仔？好运？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="还是似周润发？",
    yuewen_to_merge=['定系话自周人烦啊'],
    yuewen_merged="定系话自周人烦啊？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="不行，胶胶声，多难听！",
    yuewen_to_merge=['都系唔好', '胶胶声咁难听'],
    yuewen_merged="都系唔好，胶胶声，咁难听！",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="还是唤他麦兜！",
    yuewen_to_merge=['不如叫麦兜啦'],
    yuewen_merged="不如叫麦兜啦！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="各位⋯",
    yuewen_to_merge=['各位'],
    yuewen_merged="各位⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我就是险些给定名麦胶的小朋友⋯",
    yuewen_to_merge=['我就系呢个差少少就叫做麦胶嘅小朋友'],
    yuewen_merged="我就系呢个差少少就叫做麦胶嘅小朋友⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="麦兜！",
    yuewen_to_merge=['麦兜'],
    yuewen_merged="麦兜！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_1
# noinspection PyArgumentList
merging_test_cases_block_2 = [
MergingTestCase(
    zhongwen="麦太，没见面一阵",
    yuewen_to_merge=['咦', '麦太', '咩唔见你一排'],
    yuewen_merged="咦，麦太，咩唔见你一排",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="怎么小腿粗起来了？",
    yuewen_to_merge=['个脚刮囊粗咗咁多呀'],
    yuewen_merged="个脚刮囊粗咗咁多呀？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="可怜呀，每天扑来扑去⋯",
    yuewen_to_merge=['鬼咩', '日日扑嚟扑去'],
    yuewen_merged="鬼咩，日日扑嚟扑去⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="替儿子找幼稚园！",
    yuewen_to_merge=['同我仔揾幼稚园吖嘛'],
    yuewen_merged="同我仔揾幼稚园吖嘛！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="旧中侨国货楼上的⋯",
    yuewen_to_merge=['旧中桥百货公司楼上嗰间'],
    yuewen_merged="旧中桥百货公司楼上嗰间⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="春田花花幼稚园？",
    yuewen_to_merge=['春田花花幼稚园呢'],
    yuewen_merged="春田花花幼稚园呢？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="就是座落界限街南昌街交界⋯",
    yuewen_to_merge=['就系坐落喺界限街同南昌街交界'],
    yuewen_merged="就系坐落喺界限街同南昌街交界⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="银城美食广场附近的⋯",
    yuewen_to_merge=['银城美食广场附近嗰间'],
    yuewen_merged="银城美食广场附近嗰间⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="春田花花幼稚园？",
    yuewen_to_merge=['春田花花幼稚园呀'],
    yuewen_merged="春田花花幼稚园呀？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="对！深水埗地铁站步行不用10分钟！",
    yuewen_to_merge=['系呀', '深水埗地铁站口行过去唔使十分钟呀'],
    yuewen_merged="系呀！深水埗地铁站口行过去唔使十分钟呀！",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="春田花花幼稚园，师资优良⋯",
    yuewen_to_merge=['春田花花幼稚园', '诗诗优良'],
    yuewen_merged="春田花花幼稚园，诗诗优良⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="而且还有西人教英文！",
    yuewen_to_merge=['仲系西人教英文添㗎'],
    yuewen_merged="仲系西人教英文添㗎！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="西人教英文？",
    yuewen_to_merge=['咦', '西人教英文'],
    yuewen_merged="咦，西人教英文？",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="是呀！",
    yuewen_to_merge=['系呀'],
    yuewen_merged="系呀！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="春田花花，确有好多西人呀！",
    yuewen_to_merge=['春田花花', '真系好多西人㗎'],
    yuewen_merged="春田花花，真系好多西人㗎！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_2
# noinspection PyArgumentList
merging_test_cases_block_3 = [
MergingTestCase(
    zhongwen="这个猪样白兔小朋友⋯",
    yuewen_to_merge=['呢个扮紧白兔猪样嘅小朋友'],
    yuewen_merged="呢个扮紧白兔猪样嘅小朋友⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="横看竖看也不像发哥伟仔的一个⋯",
    yuewen_to_merge=['即系横睇掂睇都唔似发哥或者', '位仔嗰个呢'],
    yuewen_merged="即系横睇掂睇都唔似发哥或者位仔嗰个呢⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="就是我，麦兜",
    yuewen_to_merge=['就系我', '麦兜'],
    yuewen_merged="就系我，麦兜",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="这么多年来⋯",
    yuewen_to_merge=['所以咁多年嚟'],
    yuewen_merged="所以咁多年嚟⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="蛋挞！　　蛋挞！",
    yuewen_to_merge=['大湖荒岩宅'],
    yuewen_merged="大湖荒岩宅",
    difficulty=3,
    verified=True,
),
MergingTestCase(
    zhongwen="荔芋火鸭札！　　荔芋火鸭札！",
    yuewen_to_merge=['湾吉校坟交涉设'],
    yuewen_merged="湾吉校坟交涉设",
    difficulty=3,
    verified=True,
),
MergingTestCase(
    zhongwen="也不能忘记校训九十八！",
    yuewen_to_merge=['都唔好湾吉校坟交涉白'],
    yuewen_merged="都唔好湾吉校坟交涉白！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="好！各位同学⋯",
    yuewen_to_merge=['嗰个位同学'],
    yuewen_merged="嗰个位同学⋯",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="一个重要主题：",
    yuewen_to_merge=['一个可重要嘅主题'],
    yuewen_merged="一个可重要嘅主题：",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="小朋友，这个月你们交过学费没有？",
    yuewen_to_merge=['小朋友', '你哋今个月交咗学费咩呀'],
    yuewen_merged="小朋友，你哋今个月交咗学费咩呀？",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="交过了！",
    yuewen_to_merge=['交'],
    yuewen_merged="交！",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="太好了！大家去上堂吧",
    yuewen_to_merge=['哎', '好在', '噉大家可以返去上堂喇'],
    yuewen_merged="哎，好在！噉大家可以返去上堂喇",
    difficulty=2,
    prompt=True,
    verified=True,
),
]  # merging_test_cases_block_3
# noinspection PyArgumentList
merging_test_cases_block_4 = [
MergingTestCase(
    zhongwen="可是，对我和我一班同学",
    yuewen_to_merge=['但系对于我同埋我班同学仔嚟讲'],
    yuewen_merged="但系，对于我同埋我班同学仔嚟讲",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="这儿是我们最快乐，最美丽的乐园⋯",
    yuewen_to_merge=['呢度系我哋最快乐', '最美丽嘅乐园'],
    yuewen_merged="呢度系我哋最快乐，最美丽嘅乐园⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="⋯还有一个很疼我们",
    yuewen_to_merge=['仲有一个好疼我哋'],
    yuewen_merged="⋯仲有一个好疼我哋",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="就是有点游魂的Miss Chan",
    yuewen_to_merge=['不过就有少少失魂嘅班主有Miss', 'Chan'],
    yuewen_merged="不过就有少少失魂嘅班主有Miss Chan",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="我们现在开始点名",
    yuewen_to_merge=['好喇', '我哋而家开始点名'],
    yuewen_merged="好喇，我哋而家开始点名",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="麦唛同学！　　到！",
    yuewen_to_merge=['麦麦同学', '到'],
    yuewen_merged="麦麦同学！　　到！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="亚辉同学！　　到！",
    yuewen_to_merge=['阿辉同学', '到'],
    yuewen_merged="阿辉同学！　　到！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="菇时同学！　　到！",
    yuewen_to_merge=['Boosie同学', '到'],
    yuewen_merged="Boosie同学！　　到！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="得巴同学！　　到！",
    yuewen_to_merge=['德巴同学', '到'],
    yuewen_merged="德巴同学！　　到！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="阿May同学！　　到！",
    yuewen_to_merge=['阿May同学', '到'],
    yuewen_merged="阿May同学！　　到！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="阿June同学！　　到！",
    yuewen_to_merge=['阿June同学', '到'],
    yuewen_merged="阿June同学！　　到！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="阿May同学！",
    yuewen_to_merge=['阿May同学'],
    yuewen_merged="阿May同学！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="Miss Chan，我点过两次了！",
    yuewen_to_merge=['Miss', 'Chan你点咗我两次喇'],
    yuewen_merged="Miss Chan，你点咗我两次喇！",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="呀，真的吗？",
    yuewen_to_merge=['啊', '系咩'],
    yuewen_merged="啊，系咩？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我们现在继续点名",
    yuewen_to_merge=['好', '我哋而家继续点名'],
    yuewen_merged="好，我哋而家继续点名",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="菇时同学！　　到！",
    yuewen_to_merge=['川明同学', '到'],
    yuewen_merged="川明同学！　　到！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="还有谁没点过吗？",
    yuewen_to_merge=['好', '仲有边个未点'],
    yuewen_merged="好，仲有边个未点？",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="麦兜！",
    yuewen_to_merge=['猫', '噢'],
    yuewen_merged="猫噢！",
    difficulty=3,
    verified=True,
),
MergingTestCase(
    zhongwen="麦兜同学！",
    yuewen_to_merge=['麦兜同学'],
    yuewen_merged="麦兜同学！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="麦唛呀，即是呢⋯",
    yuewen_to_merge=['妈妈啊', '麦兜同学', '即系呢'],
    yuewen_merged="妈妈啊，麦兜同学，即系呢⋯",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="我好像觉得呢⋯",
    yuewen_to_merge=['我个心总系仁住仁住'],
    yuewen_merged="我个心总系仁住仁住⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="其实我正在思考一个学术问题：",
    yuewen_to_merge=['其实我系喺度思考紧一啲学术性嘅问题'],
    yuewen_merged="其实我系喺度思考紧一啲学术性嘅问题：",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="橙，为什么会是「疴－烂－煮」呢？",
    yuewen_to_merge=['点解橙叫Orange呢'],
    yuewen_merged="点解橙叫「Orange」呢？",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈说吃橙可通大便",
    yuewen_to_merge=['妈妈话食橙会通大', '变'],
    yuewen_merged="妈妈话食橙会通大变",
    verified=True,
),
MergingTestCase(
    zhongwen="「疴」这个我明白，可是「烂－煮」呢？",
    yuewen_to_merge=['噢', '呢个我明白', '但系橙呢'],
    yuewen_merged="「噢」呢个我明白，但系「橙」呢？",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="还有这个「芭－娜－哪」香蕉",
    yuewen_to_merge=['仲有呢个啊', '芭拉娜啊', '香蕉啊'],
    yuewen_merged="仲有呢个啊「芭－拉－娜」啊香蕉啊",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="为什么雨伞又会是「暗－芭－娜－哪」呢？",
    yuewen_to_merge=['点解雨姐会叫做暗芭拉娜呢'],
    yuewen_merged="点解雨姐会叫做「暗－芭－拉－娜」呢？",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="我「暗」的「暗」掉一条蕉",
    yuewen_to_merge=['嗱', '我暗啦', '噉我暗嗰条香蕉'],
    yuewen_merged="嗱，我「暗」啦，噉我「暗」嗰条香蕉",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="至多是疴烂煮，怎么会下起雨来呢？",
    yuewen_to_merge=['至多会Orange啫', '点解会搞到落雨呢'],
    yuewen_merged="至多会Orange啫，点解会搞到落雨呢？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我想，有天我念完幼稚园",
    yuewen_to_merge=['我谂', '到我读完幼稚园'],
    yuewen_merged="我谂，到我读完幼稚园",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="升小学，上中学",
    yuewen_to_merge=['识埋小学', '上到中学'],
    yuewen_merged="识埋小学，上到中学",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="再念大学⋯",
    yuewen_to_merge=['再入埋大学'],
    yuewen_merged="再入埋大学⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我知道我会明白一切！",
    yuewen_to_merge=['我谂我乜都会明白晒'],
    yuewen_merged="我谂我乜都会明白晒！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="那时候⋯",
    yuewen_to_merge=['到嗰阵'],
    yuewen_merged="到嗰阵⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我买所房子给妈妈！",
    yuewen_to_merge=['我买层楼畀我妈妈'],
    yuewen_merged="我买层楼畀我妈妈！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_4
# noinspection PyArgumentList
merging_test_cases_block_5 = [
MergingTestCase(
    zhongwen="幼稚园楼下，由校长兼营的茶餐厅",
    yuewen_to_merge=['喺幼稚园楼下校长兼营嘅间茶餐厅'],
    yuewen_merged="喺幼稚园楼下，校长兼营嘅间茶餐厅",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="鱼蛋粗面，麻烦你　　粗面买光了",
    yuewen_to_merge=['唔该鱼蛋粗啊', '冇粗面噃'],
    yuewen_merged="唔该，鱼蛋粗啊　　冇粗面噃",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="那样子⋯来碗鱼蛋河粉吧　　鱼蛋买光了",
    yuewen_to_merge=['噉啊', '要碗鱼蛋好啊', '冇鱼蛋噃'],
    yuewen_merged="噉啊⋯要碗鱼蛋好啊　　冇鱼蛋噃",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="那么⋯金钱肚粗面好了　　粗面买光了",
    yuewen_to_merge=['噉啊', '要金钱透粗啊', '冇粗面噃'],
    yuewen_merged="噉啊⋯要金钱透粗啊　　冇粗面噃",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="那么要鱼蛋油面吧　　鱼蛋买光了",
    yuewen_to_merge=['噉啊', '咁要鱼蛋油面啊', '冇鱼蛋噃'],
    yuewen_merged="噉啊咁要鱼蛋油面啊　　冇鱼蛋噃",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="怎么都买光了？",
    yuewen_to_merge=['乜样样都冇嘅'],
    yuewen_merged="乜样样都冇嘅？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="来个墨鱼丸粗面吧　　粗面买光了",
    yuewen_to_merge=['噉要蜜丸粗啊', '冇粗面噃'],
    yuewen_merged="噉要蜜丸粗啊　　冇粗面噃",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="又买光了？",
    yuewen_to_merge=['又冇啊'],
    yuewen_merged="又冇啊？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="麻烦来碗鱼蛋濑吧　　鱼蛋买光了",
    yuewen_to_merge=['噉唔该畀碗鱼蛋奶啊', '冇鱼蛋噃'],
    yuewen_merged="噉唔该畀碗鱼蛋奶啊　　冇鱼蛋噃",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="麦兜呀，鱼蛋跟粗面都买光了",
    yuewen_to_merge=['麦兜啊', '佢哋啲鱼蛋同粗面卖晒㗎啦'],
    yuewen_merged="麦兜啊，佢哋啲鱼蛋同粗面卖晒㗎啦",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="没有那些配搭吗？",
    yuewen_to_merge=['噢', '冇嗰啲配搭啊'],
    yuewen_merged="噢，冇嗰啲配搭啊？",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="麻烦你，净要鱼蛋吧　　鱼蛋买光了",
    yuewen_to_merge=['噉唔该净鱼蛋啊', '冇鱼蛋噃'],
    yuewen_merged="噉唔该，净鱼蛋啊　　冇鱼蛋噃",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="那么净要粗面呢？　　粗面买光了",
    yuewen_to_merge=['净粗面呢', '冇粗面噃'],
    yuewen_merged="净粗面呢？　　冇粗面噃",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="看到这里⋯",
    yuewen_to_merge=['睇到呢度'],
    yuewen_merged="睇到呢度⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="那时候我无忧无虑，万事无所谓－－",
    yuewen_to_merge=['果只我无忧无虑', '冇乜所谓'],
    yuewen_merged="果只我无忧无虑，冇乜所谓－－",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="鱼蛋买光了？那么粗面吧",
    yuewen_to_merge=['冇鱼蛋咩', '粗面都好啊'],
    yuewen_merged="冇鱼蛋咩？粗面都好啊",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="麦兜，射呀！",
    yuewen_to_merge=['麦兜', '转身食啊'],
    yuewen_merged="麦兜，转身食啊！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_5
# noinspection PyArgumentList
merging_test_cases_block_6 = [
MergingTestCase(
    zhongwen="看著自己每天疴烂煮⋯",
    yuewen_to_merge=['睇住自己日日柯能处'],
    yuewen_merged="睇住自己日日柯能处⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="每天长肉⋯",
    yuewen_to_merge=['日日掌肉'],
    yuewen_merged="日日掌肉⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我感到充满力量！",
    yuewen_to_merge=['感到充满力量'],
    yuewen_merged="感到充满力量！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="世界好美丽！",
    yuewen_to_merge=['世界好美丽'],
    yuewen_merged="世界好美丽！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_6
# noinspection PyArgumentList
merging_test_cases_block_7 = [
MergingTestCase(
    zhongwen="有一首歌，Miss Chan唱的好听",
    yuewen_to_merge=['有一首歌', '麦词春唱得好好听呀'],
    yuewen_merged="有一首歌，麦词春唱得好好听呀",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="可每次我总唱成「疴」什么什么的⋯",
    yuewen_to_merge=['但系唱嚟唱去都系阿伦厨', '咁Ballana噉'],
    yuewen_merged="但系唱嚟唱去都系「阿伦厨」咁「Ballana」噉⋯",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="是All Things Bright and Beautiful吧？",
    yuewen_to_merge=['系唔系All', 'Things', 'Bright', 'and', 'Beautiful呀'],
    yuewen_merged="系唔系All Things Bright and Beautiful呀？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="是的，一切都好！",
    yuewen_to_merge=['系呀', '所有嘢都几好喇'],
    yuewen_merged="系呀，所有嘢都几好喇！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="世上一切，一切一切⋯",
    yuewen_to_merge=['世上一切'],
    yuewen_merged="世上一切⋯",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="所有那些，都好！",
    yuewen_to_merge=['所有嗰啲嘢', '都几好', 'All', 'Things', 'Bright', 'and', 'Beautiful'],
    yuewen_merged="所有嗰啲嘢，都几好！All Things Bright and Beautiful",
    difficulty=2,
    verified=True,
),
]  # merging_test_cases_block_7
# noinspection PyArgumentList
merging_test_cases_block_8 = [
MergingTestCase(
    zhongwen="一、二、三、四、五、六、七⋯",
    yuewen_to_merge=['1234567'],
    yuewen_merged="1, 2, 3, 4, 5, 6, 7⋯",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="多劳多得！",
    yuewen_to_merge=['多喽多得'],
    yuewen_merged="多喽多得！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="星期一至星期七⋯多劳多得！",
    yuewen_to_merge=['星期一至星期七多喽多得'],
    yuewen_merged="星期一至星期七⋯多喽多得！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="就是我妈妈麦太",
    yuewen_to_merge=['就系我妈妈', '麦太'],
    yuewen_merged="就系我妈妈麦太",
    verified=True,
),
MergingTestCase(
    zhongwen="一个女人背起整个世界！",
    yuewen_to_merge=['一个女人揹起成个世界'],
    yuewen_merged="一个女人揹起成个世界！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_8
# noinspection PyArgumentList
merging_test_cases_block_9 = [
MergingTestCase(
    zhongwen="是的，我妈妈真的很厉害",
    yuewen_to_merge=['系呀', '我妈妈真系好犀利㗎'],
    yuewen_merged="系呀，我妈妈真系好犀利㗎",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="除了兼任保险，地产经纪及trading⋯",
    yuewen_to_merge=['除咗做保险地产经纪同埋trading之外'],
    yuewen_merged="除咗做保险，地产经纪同埋trading之外⋯",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="她还趁高科技热潮搞了个烹饪网站⋯",
    yuewen_to_merge=['佢仲趁住高科技热潮搞咗个煮𩠌嘅网站'],
    yuewen_merged="佢仲趁住高科技热潮搞咗个煮𩠌嘅网站⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="www．麦太世界．com",
    yuewen_to_merge=['www.mcticege.com'],
    yuewen_merged="www．mcticege．com",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="她做的菜，同样厉害",
    yuewen_to_merge=['佢煮嘅𩠌一样咁犀利'],
    yuewen_merged="佢煮嘅𩠌，一样咁犀利",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="欢迎大家收看＜麦太世界＞",
    yuewen_to_merge=['欢迎大家收睇麦太世界'],
    yuewen_merged="欢迎大家收睇＜麦太世界＞",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="今日为大家介绍一个⋯",
    yuewen_to_merge=['今日我为大家介绍个'],
    yuewen_merged="今日我为大家介绍个⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="简单别致的小菜纸包鸡",
    yuewen_to_merge=['简单又别致嘅小菜', '自包鸡'],
    yuewen_merged="简单又别致嘅小菜自包鸡",
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="材料很简单：一个鸡包",
    yuewen_to_merge=['材料系好简单', '我哋只需要一个鸡包'],
    yuewen_merged="材料系好简单：我哋只需要一个鸡包",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="将鸡包底部的纸撕下来⋯慢慢地撕",
    yuewen_to_merge=['我哋将黐喺鸡包底嘅纸撕出嚟', '慢慢撕'],
    yuewen_merged="我哋将黐喺鸡包底嘅纸撕出嚟⋯慢慢撕",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="把鸡包纸一反反转",
    yuewen_to_merge=['然后将鸡包纸一反', '反转'],
    yuewen_merged="然后将鸡包纸一反反转",
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="这一味纸包鸡就完成了，很容易是吧？",
    yuewen_to_merge=['呢味自包鸡就完成喇', '系咪好易整啦'],
    yuewen_merged="呢味自包鸡就完成喇，系咪好易整啦？",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_9
# noinspection PyArgumentList
merging_test_cases_block_10 = [
MergingTestCase(
    zhongwen="材料也很简单，只需要白纸一张",
    yuewen_to_merge=['材料都好简单', '只需要白纸一张'],
    yuewen_merged="材料都好简单，只需要白纸一张",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="只要把这纸这样子⋯",
    yuewen_to_merge=['我哋只需要张张纸', '咁样'],
    yuewen_merged="我哋只需要张张纸咁样⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="各位小朋友，像鸡包不像呀？",
    yuewen_to_merge=['各位小朋友', '你哋话似唔似鸡包啊'],
    yuewen_merged="各位小朋友，你哋话似唔似鸡包啊？",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_10
# noinspection PyArgumentList
merging_test_cases_block_11 = [
MergingTestCase(
    zhongwen="现在要教大家一味别致小菜－",
    yuewen_to_merge=['间阵要教大家一味几别节嘅小菜'],
    yuewen_merged="间阵要教大家一味几别节嘅小菜－",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="包鸡纸包鸡包纸包鸡",
    yuewen_to_merge=['包鸡子包', '鸡包子包鸡'],
    yuewen_merged="包鸡子包鸡包子包鸡",
    verified=True,
),
MergingTestCase(
    zhongwen="一味「包鸡纸包鸡包纸包鸡」完成了！",
    yuewen_to_merge=['咁一味包鸡子包鸡包子包鸡就完成喇'],
    yuewen_merged="咁一味「包鸡子包鸡包子包鸡」就完成喇！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="真的很简单吧？",
    yuewen_to_merge=['系咪好简单呢'],
    yuewen_merged="系咪好简单呢？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="还真有一块鸡吃呢！",
    yuewen_to_merge=['仲真系有嚿鸡食添'],
    yuewen_merged="仲真系有嚿鸡食添！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_11
# noinspection PyArgumentList
merging_test_cases_block_12 = [
MergingTestCase(
    zhongwen="今日为大家介绍一味⋯",
    yuewen_to_merge=['今日要同大家介绍一味'],
    yuewen_merged="今日要同大家介绍一味⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="小朋友一定喜欢的⋯",
    yuewen_to_merge=['小朋友一定喜欢嘅'],
    yuewen_merged="小朋友一定喜欢嘅⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="鸡包包鸡包包鸡包纸包纸⋯",
    yuewen_to_merge=['鸡包包', '鸡包包', '鸡包纸包', '纸包鸡', '包包鸡'],
    yuewen_merged="鸡包包鸡包包鸡包纸包纸包鸡包包鸡⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="包鸡包鸡包纸包鸡",
    yuewen_to_merge=['纸包鸡', '包纸包鸡'],
    yuewen_merged="纸包鸡包纸包鸡",
    verified=True,
),
MergingTestCase(
    zhongwen="只要将鸡包包住个鸡包",
    yuewen_to_merge=['我哋先将鸡包', '包住个鸡包'],
    yuewen_merged="我哋先将鸡包包住个鸡包",
    verified=True,
),
MergingTestCase(
    zhongwen="再包住个鸡包⋯",
    yuewen_to_merge=['再包住个鸡包'],
    yuewen_merged="再包住个鸡包⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="再包包包包包住个纸鸡包",
    yuewen_to_merge=['再包包包包', '包住个纸包鸡'],
    yuewen_merged="再包包包包包住个纸包鸡",
    verified=True,
),
MergingTestCase(
    zhongwen="再包包包，纸纸纸",
    yuewen_to_merge=['再包包包包', '包鸡包纸', '包纸', '纸纸纸纸', '纸包纸'],
    yuewen_merged="再包包包包包鸡包纸包纸，纸纸纸纸纸包纸",
    difficulty=3,
    verified=True,
),
MergingTestCase(
    zhongwen="纸包纸，纸包鸡，鸡包纸，纸包鸡⋯",
    yuewen_to_merge=['纸包鸡', '包鸡纸', '纸包鸡', '鸡鸡鸡', '纸纸纸再包鸡鸡'],
    yuewen_merged="纸包鸡，包鸡纸，纸包鸡，鸡鸡鸡，纸纸纸再包鸡鸡⋯",
    difficulty=3,
    verified=True,
),
MergingTestCase(
    zhongwen="例如每晚睡觉前，她都会说故事给我听",
    yuewen_to_merge=['例如每晚临睡前', '佢都会讲故事畀我听'],
    yuewen_merged="例如每晚临睡前，佢都会讲故事畀我听",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="从前，有个小朋友撒谎；有一天⋯",
    yuewen_to_merge=['从前有个小朋友讲大话', '有一日'],
    yuewen_merged="从前，有个小朋友讲大话；有一日⋯",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="他死了！",
    yuewen_to_merge=['佢死咗'],
    yuewen_merged="佢死咗！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="从前，有个小朋友很勤力念书⋯",
    yuewen_to_merge=['从前有个小朋友好勤力读书'],
    yuewen_merged="从前，有个小朋友好勤力读书⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="他长大后，发财了！",
    yuewen_to_merge=['佢长大发咗'],
    yuewen_merged="佢长大，发咗！",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="从前，有个小朋友不孝，有天⋯",
    yuewen_to_merge=['从前有个小朋友唔孝顺', '有一日'],
    yuewen_merged="从前，有个小朋友唔孝顺，有一日⋯",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="他扭了脚骹！",
    yuewen_to_merge=['佢屈亲个脚脚'],
    yuewen_merged="佢屈亲个脚脚！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈，我想睡觉",
    yuewen_to_merge=['妈我想瞓啦'],
    yuewen_merged="妈，我想瞓啦",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_12
# noinspection PyArgumentList
merging_test_cases_block_13 = [
MergingTestCase(
    zhongwen="从前，有个小朋友早睡晚起；第二天⋯",
    yuewen_to_merge=['从前有个小朋友早睡晚起', '第二朝', '社群提供'],
    yuewen_merged="从前，有个小朋友早睡晚起；第二朝⋯社群提供",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="他死了！",
    yuewen_to_merge=['佢死咗'],
    yuewen_merged="佢死咗！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我妈妈就是这样子，一切都那么直接",
    yuewen_to_merge=['我妈妈就系噉', '一切都咁直接'],
    yuewen_merged="我妈妈就系噉，一切都咁直接",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="她爱得我直接⋯",
    yuewen_to_merge=['佢爱得我直接'],
    yuewen_merged="佢爱得我直接⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="对她，一、二、三、四、五、六、七",
    yuewen_to_merge=['佢一二三四五六七'],
    yuewen_merged="佢，一、二、三、四、五、六、七",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="没有不成的事",
    yuewen_to_merge=['唔得都要得', '字幕由', 'Amara.org'],
    yuewen_merged="唔得都要得字幕由Amara.org",
    difficulty=2,
    verified=True,
),
]  # merging_test_cases_block_13
# noinspection PyArgumentList
merging_test_cases_block_14 = [
MergingTestCase(
    zhongwen="可有些事情，要是真的不成呢？",
    yuewen_to_merge=['但系如果有啲嘢真系真系唔得呢'],
    yuewen_merged="但系如果有啲嘢，真系真系唔得呢？",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="首先是周润发事件⋯",
    yuewen_to_merge=['首先周润发嗰单嘢'],
    yuewen_merged="首先周润发嗰单嘢⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="拜托不要再提了！",
    yuewen_to_merge=['大家都唔好再提'],
    yuewen_merged="大家都唔好再提！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="至于好运⋯",
    yuewen_to_merge=['至于好运'],
    yuewen_merged="至于好运⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="奇迹般似的一个号码也没中过！",
    yuewen_to_merge=['竟然奇迹一样', '一个都未中过'],
    yuewen_merged="竟然奇迹一样一个都未中过！",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="叻仔？",
    yuewen_to_merge=['叻仔'],
    yuewen_merged="叻仔？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我也试过努力念书，可是⋯",
    yuewen_to_merge=['我试过好努力咁读书', '但系'],
    yuewen_merged="我试过好努力咁读书，但系⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="可是⋯我仍然有梦",
    yuewen_to_merge=['但系我仲可以发梦'],
    yuewen_merged="但系⋯我仲可以发梦",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_14
# noinspection PyArgumentList
merging_test_cases_block_15 = [
MergingTestCase(
    zhongwen="马尔代夫，座落于印度洋的世外桃源",
    yuewen_to_merge=['马尔代夫', '坐落于印度洋嘅世外桃源'],
    yuewen_merged="马尔代夫，坐落于印度洋嘅世外桃源",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="蓝天白云，椰林树影，水清沙幼",
    yuewen_to_merge=['蓝天白云', '椰林树影', '水清沙游'],
    yuewen_merged="蓝天白云，椰林树影，水清沙游",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="七彩缤纷的珊瑚，目不暇给的热带鱼",
    yuewen_to_merge=['七彩缤纷嘅珊瑚', '目不下级嘅热带鱼群'],
    yuewen_merged="七彩缤纷嘅珊瑚，目不下级嘅热带鱼群",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="充满赤道活力的原始海洋，脱离繁嚣",
    yuewen_to_merge=['充满住赤道热力嘅原始海洋', '远离凡嚣'],
    yuewen_merged="充满住赤道热力嘅原始海洋，远离凡嚣",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="犀利旅行社，旅行社牌照号码350999",
    yuewen_to_merge=['犀利旅行社', '旅行社牌照号码350999'],
    yuewen_merged="犀利旅行社，旅行社牌照号码350999",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈你知道马尔代夫在哪儿吗？",
    yuewen_to_merge=['妈妈你知唔知到马尔代夫系边㗎'],
    yuewen_merged="妈妈你知唔知到马尔代夫系边㗎？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="有多远？",
    yuewen_to_merge=['点远发呀'],
    yuewen_merged="点远发呀？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈你会带我去吗？",
    yuewen_to_merge=['咁妈妈你会唔会走落去㗎'],
    yuewen_merged="咁妈妈你会唔会走落去㗎？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="会！发财了再说吧",
    yuewen_to_merge=['会', '得学发咗先啦'],
    yuewen_merged="会！得学发咗先啦",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="那么妈妈你什么时候发？",
    yuewen_to_merge=['咁妈妈你几时发得呀'],
    yuewen_merged="咁妈妈你几时发得呀？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="快了⋯",
    yuewen_to_merge=['呃', '就快啦'],
    yuewen_merged="呃，就快啦⋯",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="发梦呀！",
    yuewen_to_merge=['发梦吖嘛'],
    yuewen_merged="发梦吖嘛！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_15
# noinspection PyArgumentList
merging_test_cases_block_16 = [
MergingTestCase(
    zhongwen="校长早晨！",
    yuewen_to_merge=['嗨'],
    yuewen_merged="嗨！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="你最喜爱的地方是哪儿？",
    yuewen_to_merge=['你最喜爱嘅地方喺边度呀'],
    yuewen_merged="你最喜爱嘅地方喺边度呀？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我最喜爱的地方是日本",
    yuewen_to_merge=['我最喜爱嘅地方呢', '就系日本喇'],
    yuewen_merged="我最喜爱嘅地方呢，就系日本喇",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="那儿有 Disneyland 和 Hello Kitty Land",
    yuewen_to_merge=['嗰度好迪士尼呢', '同埋HelloTT呢'],
    yuewen_merged="嗰度好迪士尼呢同埋HelloTT呢",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="婆婆跟舅父他们都在加拿大",
    yuewen_to_merge=['婆婆同埋舅父呀', '佢哋都系加拿大㗎'],
    yuewen_merged="婆婆同埋舅父呀，佢哋都系加拿大㗎",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="那儿有很好多水上活动，还有鱼翅吃",
    yuewen_to_merge=['嗰度有好多水晶活动㗎', '仲有一次食添㖞'],
    yuewen_merged="嗰度有好多水晶活动㗎，仲有一次食添㖞",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="我最喜爱的地方⋯",
    yuewen_to_merge=['呃', '我最喜爱嘅地方呢'],
    yuewen_merged="呃，我最喜爱嘅地方呢⋯",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="就是那间什么！",
    yuewen_to_merge=['就系嗰间咩嚟啰'],
    yuewen_merged="就系嗰间咩嚟啰！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="那儿有欢乐天地，还有美食广场",
    yuewen_to_merge=['嗰度有欢乐天地啦', '仲有米食广场啦'],
    yuewen_merged="嗰度有欢乐天地啦，仲有米食广场啦",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="对了，那地方叫银城中心",
    yuewen_to_merge=['系喇系喇', '嗰间叫做银城中心'],
    yuewen_merged="系喇系喇，嗰间叫做银城中心",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="那店子的饭很多，很大碟的！",
    yuewen_to_merge=['嗰间嘢啲饭好多人', '好大碟㗎'],
    yuewen_merged="嗰间嘢啲饭好多人，好大碟㗎！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="不过说到我最想去的地方，那可厉害了",
    yuewen_to_merge=['不过讲到我最想去嘅地方呢', '嗰度细嚟啰'],
    yuewen_merged="不过讲到我最想去嘅地方呢，嗰度细嚟啰",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="那儿蓝天白云，椰林树影，水清沙幼",
    yuewen_to_merge=['嗰度南天白云', '夜临树影', '水清沙幽'],
    yuewen_merged="嗰度南天白云，夜临树影，水清沙幽",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="座落于印度洋的世外桃源",
    yuewen_to_merge=['独来鱼', '印度洋嘅世外桃源'],
    yuewen_merged="独来鱼印度洋嘅世外桃源",
    prompt=True,
    verified=True,
),
]  # merging_test_cases_block_16
# noinspection PyArgumentList
merging_test_cases_block_17 = [
MergingTestCase(
    zhongwen="衰仔，快点起床上学",
    yuewen_to_merge=['喂', '衰仔啊', '快啲起身返学喇'],
    yuewen_merged="喂，衰仔啊，快啲起身返学喇",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="咦？",
    yuewen_to_merge=['咦'],
    yuewen_merged="咦？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈！",
    yuewen_to_merge=['妈妈'],
    yuewen_merged="妈妈！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_17
# noinspection PyArgumentList
merging_test_cases_block_18 = [
MergingTestCase(
    zhongwen="医生，吃了药会不会有那个什么的？",
    yuewen_to_merge=['医生啊', '啲药食咗会唔会有嗰啲咩㗎'],
    yuewen_merged="医生啊，啲药食咗会唔会有嗰啲咩㗎？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="不会！",
    yuewen_to_merge=['唔会'],
    yuewen_merged="唔会！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="那么吃药用不用那个什么的？",
    yuewen_to_merge=['噉佢食药使唔使咩啊'],
    yuewen_merged="噉佢食药使唔使咩啊？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="不用！给他打口针吧！",
    yuewen_to_merge=['唔使', '同佢打多支针添呢'],
    yuewen_merged="唔使！同佢打多支针添呢！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="怎么？得打针？",
    yuewen_to_merge=['吓', '要打针啊'],
    yuewen_merged="吓？要打针啊？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="那么他怕不怕死？",
    yuewen_to_merge=['噉佢怕唔怕死呀'],
    yuewen_merged="噉佢怕唔怕死呀？",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_18
# noinspection PyArgumentList
merging_test_cases_block_19 = [
MergingTestCase(
    zhongwen="没事吧？快点先把药水喝掉！",
    yuewen_to_merge=['冇嘢吖嘛', '快啲食埋啲药水佢先啦'],
    yuewen_merged="冇嘢吖嘛？快啲食埋啲药水佢先啦！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈我不想喝药水",
    yuewen_to_merge=['妈妈', '我唔想食药水呀'],
    yuewen_merged="妈妈我唔想食药水呀",
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="不要呀妈妈，我不喝呀",
    yuewen_to_merge=['唔好捞妈妈', '我唔食呀'],
    yuewen_merged="唔好捞妈妈，我唔食呀",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我不喝士多啤梨药水呀！",
    yuewen_to_merge=['我唔食士多啤梨药水呀'],
    yuewen_merged="我唔食士多啤梨药水呀！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="别哭了，不喝药水病不会好的",
    yuewen_to_merge=['唔好喊啦', '唔食药唔会好㗎'],
    yuewen_merged="唔好喊啦，唔食药唔会好㗎",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="乖乖，病好了妈妈带你去马尔代夫",
    yuewen_to_merge=['乖乖啲', '病好咗', '妈妈大理马尔代夫'],
    yuewen_merged="乖乖啲，病好咗妈妈大理马尔代夫",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="真的吗？",
    yuewen_to_merge=['真嘅'],
    yuewen_merged="真嘅？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈什么时候骗过你？",
    yuewen_to_merge=['妈妈几时呃过你呀'],
    yuewen_merged="妈妈几时呃过你呀？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="乖，先把药水喝掉",
    yuewen_to_merge=['乖', '食埋啲药水先啦'],
    yuewen_merged="乖，食埋啲药水先啦",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_19
# noinspection PyArgumentList
merging_test_cases_block_20 = [
MergingTestCase(
    zhongwen="好呀，马尔代夫！",
    yuewen_to_merge=['嘻嘻', '好嘢', '买二代夫'],
    yuewen_merged="嘻嘻，好嘢，买二代夫！",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="马尔代夫！",
    yuewen_to_merge=['买二代夫'],
    yuewen_merged="买二代夫！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈，那么我们什么时候去？",
    yuewen_to_merge=['妈妈', '咁我哋几时去呀'],
    yuewen_merged="妈妈，咁我哋几时去呀？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="你先把药水喝掉，病好了我就去订机票",
    yuewen_to_merge=['嗯', '你乖乖哋食埋啲药', '好返晒啦', '我即刻订机票'],
    yuewen_merged="嗯，你乖乖哋食埋啲药，好返晒啦我即刻订机票",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="来，多喝一点！",
    yuewen_to_merge=['嚟啦', '食多更'],
    yuewen_merged="嚟啦，食多更！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_20
# noinspection PyArgumentList
merging_test_cases_block_21 = [
MergingTestCase(
    zhongwen="妈妈你看，我病好了！",
    yuewen_to_merge=['妈妈你睇', '我好返喇'],
    yuewen_merged="妈妈你睇，我好返喇！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="家中的东西有什么没给你吃光的？",
    yuewen_to_merge=['即系间屋有乜嘢唔系畀你食晒㗎'],
    yuewen_merged="即系间屋有乜嘢唔系畀你食晒㗎？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="这次不同呀，原来这么一大樽的",
    yuewen_to_merge=['妈妈', '呢次唔同㗎', '本来咁大樽嘅'],
    yuewen_merged="妈妈，呢次唔同㗎，本来咁大樽嘅",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="我喝一格，又喝一格，又喝一格⋯",
    yuewen_to_merge=['我饮下一格', '又一格', '又一格'],
    yuewen_merged="我饮下一格，又一格，又一格⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="就给我喝光了！",
    yuewen_to_merge=['吓', '咪我饮晒㖞'],
    yuewen_merged="吓，咪我饮晒㖞！",
    difficulty=2,
    verified=True,
),
]  # merging_test_cases_block_21
# noinspection PyArgumentList
merging_test_cases_block_22 = [
MergingTestCase(
    zhongwen="喝光了就叻仔了！",
    yuewen_to_merge=['饮实就叻仔啦'],
    yuewen_merged="饮实就叻仔啦！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="喝光了就病好了！",
    yuewen_to_merge=['饮实就好返实啦'],
    yuewen_merged="饮实就好返实啦！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈呀⋯",
    yuewen_to_merge=['妈妈'],
    yuewen_merged="妈妈⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="什么事？",
    yuewen_to_merge=['乜嘢啊'],
    yuewen_merged="乜嘢啊？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我们什么时候去马尔代夫？",
    yuewen_to_merge=['阿哋几时去马尔代夫啊'],
    yuewen_merged="阿哋几时去马尔代夫啊？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="什么马尔代夫？",
    yuewen_to_merge=['乜嘢马尔代夫啊'],
    yuewen_merged="乜嘢马尔代夫啊？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="你说我病好带我去马尔代夫的呀！",
    yuewen_to_merge=['呢', '你话我返就同我去马尔代夫㗎嘛'],
    yuewen_merged="呢，你话我返就同我去马尔代夫㗎嘛！",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="马尔代夫，椰林树影，水清沙幼⋯",
    yuewen_to_merge=['马尔代夫呢', '耶南树影', '水清沙游'],
    yuewen_merged="马尔代夫呢，耶南树影，水清沙游⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="座落于印度洋的世外桃源呀！",
    yuewen_to_merge=['助流于印度园嘅世外导演啦'],
    yuewen_merged="助流于印度园嘅世外导演啦！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="想不到你还有点文采",
    yuewen_to_merge=['啊', '估唔到你几好文采㗎噃'],
    yuewen_merged="啊，估唔到你几好文采㗎噃",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="说得不错呀！",
    yuewen_to_merge=['讲得几好听啊'],
    yuewen_merged="讲得几好听啊！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我不是光说的呀，妈妈你说过⋯",
    yuewen_to_merge=['妈妈我唔系讲喇㗎', '又系你话嘅'],
    yuewen_merged="妈妈我唔系讲喇㗎，又系你话嘅⋯",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="我病好了带我去马尔代夫的！",
    yuewen_to_merge=['你话我病好咗之日就同我去马尔代夫㗎', '你讲过㗎'],
    yuewen_merged="你话我病好咗之日就同我去马尔代夫㗎！你讲过㗎！",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="不是的，妈妈你说我病好了就去的",
    yuewen_to_merge=['唔系㖞', '妈妈', '你话好咗就同我去㗎㖞'],
    yuewen_merged="唔系㖞，妈妈你话好咗就同我去㗎㖞",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="你讲过的！",
    yuewen_to_merge=['你讲过㗎'],
    yuewen_merged="你讲过㗎！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="好了，别哭了",
    yuewen_to_merge=['得啦得啦', '唔好喊啦'],
    yuewen_merged="得啦得啦，唔好喊啦",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="真的吗？　　对",
    yuewen_to_merge=['真嘅', '系啊'],
    yuewen_merged="真嘅？　　系啊",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="什么时候去？",
    yuewen_to_merge=['咁几时去啊'],
    yuewen_merged="咁几时去啊？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="你早发财了⋯",
    yuewen_to_merge=['你发咗㗎喇', '你发咗㗎喇'],
    yuewen_merged="你发咗㗎喇，你发咗㗎喇⋯",
    difficulty=2,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="好了好了，发财了",
    yuewen_to_merge=['系喇系喇系喇发咗喇'],
    yuewen_merged="系喇系喇系喇，发咗喇",
    difficulty=1,
    prompt=True,
    verified=True,
),
MergingTestCase(
    zhongwen="我们下个星期去，好了吧？",
    yuewen_to_merge=['下个礼拜同你去啦', '得未啊'],
    yuewen_merged="下个礼拜同你去啦，得未啊？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="太好了！",
    yuewen_to_merge=['好嘢'],
    yuewen_merged="好嘢！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_22
# noinspection PyArgumentList
merging_test_cases_block_23 = [
MergingTestCase(
    zhongwen="麦唛，我是麦兜呀",
    yuewen_to_merge=['喂', '麦麦啊', '麦豆啊我系'],
    yuewen_merged="喂，麦麦啊，麦豆啊我系",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="是这样子的，我明天就飞了",
    yuewen_to_merge=['即系呢我明日就要飞喇'],
    yuewen_merged="即系呢，我明日就要飞喇",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="对　　⋯是吗？",
    yuewen_to_merge=['系啊', '系咩'],
    yuewen_merged="系啊　　⋯系咩？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="飞机餐很难吃的吗？",
    yuewen_to_merge=['飞机真好难食㗎'],
    yuewen_merged="飞机真好难食㗎？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="也得吃呀！",
    yuewen_to_merge=['但点都要食㗎啦'],
    yuewen_merged="但点都要食㗎啦！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="难道自己带东西上去吃吗？",
    yuewen_to_merge=['唔通自己带嘢上去食咩'],
    yuewen_merged="唔通自己带嘢上去食咩？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="还在讲？",
    yuewen_to_merge=['仲讲', '哦'],
    yuewen_merged="仲讲哦？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="快点帮手执行李",
    yuewen_to_merge=['快啲嚟执埋啲行李先啦', '哦'],
    yuewen_merged="快啲嚟执埋啲行李先啦哦",
    verified=True,
),
MergingTestCase(
    zhongwen="跟我向他们说，我明天去马尔代夫了",
    yuewen_to_merge=['你帮我话畀佢哋知', '我明日去买热带裤薄'],
    yuewen_merged="你帮我话畀佢哋知，我明日去买热带裤薄",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="那边蓝天白云，椰林树影⋯",
    yuewen_to_merge=['嗰度蓝天五百云', '夜临雪'],
    yuewen_merged="嗰度蓝天五百云，夜临雪⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="还在讲！",
    yuewen_to_merge=['水清净沙有'],
    yuewen_merged="水清净沙有！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="来了！",
    yuewen_to_merge=['我嚟紧㗎喇'],
    yuewen_merged="我嚟紧㗎喇！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="要执行李了，回来再跟你说吧",
    yuewen_to_merge=['我要执行你喇', '返嚟先再同你倾过啦'],
    yuewen_merged="我要执行你喇，返嚟先再同你倾过啦",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="再见！",
    yuewen_to_merge=['拜拜'],
    yuewen_merged="拜拜！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_23
# noinspection PyArgumentList
merging_test_cases_block_24 = [
MergingTestCase(
    zhongwen="妈妈，我得把出世纸带著吗？",
    yuewen_to_merge=['哎哟', '妈妈', '我系咪要带埋出世纸去㗎'],
    yuewen_merged="哎哟，妈妈，我系咪要带埋出世纸去㗎？",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="那么成绩表呢？",
    yuewen_to_merge=['咁成绩表呢'],
    yuewen_merged="咁成绩表呢？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="太好了！吓得我！",
    yuewen_to_merge=['好嘢', '吓得我啊', '咁都好啲'],
    yuewen_merged="好嘢！吓得我啊！咁都好啲",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_24
# noinspection PyArgumentList
merging_test_cases_block_25 = [
MergingTestCase(
    zhongwen="找到了！",
    yuewen_to_merge=['揾到喇'],
    yuewen_merged="揾到喇！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="出世纸给我找到了！",
    yuewen_to_merge=['竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然'],
    yuewen_merged="竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然竟然！",
    difficulty=3,
    verified=True,
),
]  # merging_test_cases_block_25
# noinspection PyArgumentList
merging_test_cases_block_26 = [
MergingTestCase(
    zhongwen="早机去，晚机返",
    yuewen_to_merge=['早机去', '晚机返'],
    yuewen_merged="早机去，晚机返",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="就这样⋯",
    yuewen_to_merge=['就系噉样'],
    yuewen_merged="就系噉样⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我过了我小时候最精明⋯",
    yuewen_to_merge=['我过咗我小时候最著数'],
    yuewen_merged="我过咗我小时候最著数⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="依你说，纸是否可以包著鸡呢？",
    yuewen_to_merge=['噉你话纸包唔包得绝鸡呢'],
    yuewen_merged="噉你话，纸包唔包得绝鸡呢？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="也可以的⋯",
    yuewen_to_merge=['都得嘅'],
    yuewen_merged="都得嘅⋯",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_26
# noinspection PyArgumentList
merging_test_cases_block_27 = [
MergingTestCase(
    zhongwen="妈妈晚安！",
    yuewen_to_merge=['妈妈', '做头'],
    yuewen_merged="妈妈做头！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_27
# noinspection PyArgumentList
merging_test_cases_block_28 = [
MergingTestCase(
    zhongwen="夺得香港历史上第一面奥运金牌！",
    yuewen_to_merge=['夺取香港历史上第一面奥运金牌'],
    yuewen_merged="夺取香港历史上第一面奥运金牌！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="激动地对在场记者表示她今次的成绩⋯",
    yuewen_to_merge=['好激动噉同在场嘅记者讲', '今次佢嘅成绩'],
    yuewen_merged="好激动噉同在场嘅记者讲今次佢嘅成绩⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="足以证明香港运动员不是腊鸭！",
    yuewen_to_merge=['可以证明到香港嘅运动员唔系𫚭鸭'],
    yuewen_merged="可以证明到香港嘅运动员唔系𫚭鸭！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="对不起，应该　　是垃圾，不是腊鸭！",
    yuewen_to_merge=['各位对唔住', '应该系垃圾', '唔系𫚭鸭'],
    yuewen_merged="各位对唔住，应该　　系垃圾，唔系𫚭鸭！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="对不起，应该　　不是垃圾，也不是腊鸭！",
    yuewen_to_merge=['对唔住', '应该系唔系垃圾', '亦都唔系𫚭鸭'],
    yuewen_merged="对唔住，应该系　　唔系垃圾，亦都唔系𫚭鸭！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_28
# noinspection PyArgumentList
merging_test_cases_block_29 = [
MergingTestCase(
    zhongwen="妈妈好像又有计了",
    yuewen_to_merge=['咦', '妈妈好似又有计噉噃'],
    yuewen_merged="咦，妈妈好似又有计噉噃",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="靓仔，好运，叻仔⋯",
    yuewen_to_merge=['靓仔', '好运', '叻仔呀'],
    yuewen_merged="靓仔，好运，叻仔呀⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="是不是可以靠手瓜呢？",
    yuewen_to_merge=['哗', '好唔好靠下个手瓜噉呢'],
    yuewen_merged="哗，好唔好靠下个手瓜噉呢？",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="于是，一个梦还没醒⋯",
    yuewen_to_merge=['于是', '一个梦都未醒'],
    yuewen_merged="于是，一个梦都未醒⋯",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_29
# noinspection PyArgumentList
merging_test_cases_block_30 = [
MergingTestCase(
    zhongwen="但无论多不容易，我都要试一试",
    yuewen_to_merge=['但无论几唔容易', '我都要试一试'],
    yuewen_merged="但无论几唔容易，我都要试一试",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我要黎根收我做徒弟！",
    yuewen_to_merge=['我要来紧收我度徒弟'],
    yuewen_merged="我要来紧收我度徒弟！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="无论几辛苦，我一定要得到奥运金牌！",
    yuewen_to_merge=['无论几辛苦', '我一定要捞到奥运金牌'],
    yuewen_merged="无论几辛苦，我一定要捞到奥运金牌！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_30
# noinspection PyArgumentList
merging_test_cases_block_31 = [
MergingTestCase(
    zhongwen="我会举起金牌跟全世界说：",
    yuewen_to_merge=['系今排同全世界讲'],
    yuewen_merged="系今排同全世界讲：",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_31
# noinspection PyArgumentList
merging_test_cases_block_32 = [
MergingTestCase(
    zhongwen="长洲，我终于来到长洲了！",
    yuewen_to_merge=['长洲', '我终于嚟到长洲嘞'],
    yuewen_merged="长洲，我终于嚟到长洲嘞！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_32
# noinspection PyArgumentList
merging_test_cases_block_33 = [
MergingTestCase(
    zhongwen="长洲，我得亲吻这片圣洁的土地！",
    yuewen_to_merge=['长洲', '我要亲吻呢片盛洁嘅土地'],
    yuewen_merged="长洲，我要亲吻呢片盛洁嘅土地！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_33
# noinspection PyArgumentList
merging_test_cases_block_34 = [
MergingTestCase(
    zhongwen="小朋友，这儿是南丫岛呀！",
    yuewen_to_merge=['小朋友呀', '呢度系南丫岛噃'],
    yuewen_merged="小朋友呀，呢度系南丫岛噃！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="南丫岛？它也孕育了周润发！",
    yuewen_to_merge=['南丫岛', '都引用咗周润发噃'],
    yuewen_merged="南丫岛？都引用咗周润发噃！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_34
# noinspection PyArgumentList
merging_test_cases_block_35 = [
MergingTestCase(
    zhongwen="小朋友，你知道什么是狗仔队吧？",
    yuewen_to_merge=['小朋友', '我谂你都知道乜嘢叫做狗仔队嘞'],
    yuewen_merged="小朋友，我谂你都知道乜嘢叫做狗仔队嘞？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="至于拜师的事⋯",
    yuewen_to_merge=['至于拜师嘅嘢'],
    yuewen_merged="至于拜师嘅嘢⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="拜你个头！",
    yuewen_to_merge=['拜你个头嘅'],
    yuewen_merged="拜你个头嘅！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="怎么吃得苦？",
    yuewen_to_merge=['边挨得苦㗎'],
    yuewen_merged="边挨得苦㗎？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="想跟珊珊般得奥运金牌？",
    yuewen_to_merge=['想学山伞攞奥运金牌'],
    yuewen_merged="想学山伞攞奥运金牌？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="别作梦了！",
    yuewen_to_merge=['食母你嘢'],
    yuewen_merged="食母你嘢！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_35
# noinspection PyArgumentList
merging_test_cases_block_36 = [
MergingTestCase(
    zhongwen="小朋友，你看！",
    yuewen_to_merge=['小朋友', '你睇下'],
    yuewen_merged="小朋友，你睇下！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_36
# noinspection PyArgumentList
merging_test_cases_block_37 = [
MergingTestCase(
    zhongwen="这个⋯",
    yuewen_to_merge=['哗', '呢只'],
    yuewen_merged="哗，呢只⋯",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="这脚瓜⋯好粗好大！比一节瓜还要大！",
    yuewen_to_merge=['呢只脚瓜好粗好大呀', '仲大个只瓜呀'],
    yuewen_merged="呢只脚瓜⋯好粗好大呀！仲大个只瓜呀！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="脚瓜的肌肉非常结实⋯",
    yuewen_to_merge=['脚瓜啲肌肉非常结实'],
    yuewen_merged="脚瓜啲肌肉非常结实⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="青筋凸现，钢线似的",
    yuewen_to_merge=['啲青筋凸晒出嚟', '好似钢线噉'],
    yuewen_merged="啲青筋凸晒出嚟，好似钢线噉",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="每一条脚毛都硬似铁钉",
    yuewen_to_merge=['啲脚毛', '每一条好似铁钉噉硬'],
    yuewen_merged="啲脚毛，每一条好似铁钉噉硬",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="脚趾甲有一寸厚，究竟⋯",
    yuewen_to_merge=['脚趾弓啲脚甲成吋噉厚', '究竟'],
    yuewen_merged="脚趾弓啲脚甲成吋噉厚，究竟⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="要行过几多座山⋯",
    yuewen_to_merge=['要行我几多座山'],
    yuewen_merged="要行我几多座山⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="跨过几多个海⋯",
    yuewen_to_merge=['跨我几多个海'],
    yuewen_merged="跨我几多个海⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="吃过几多苦头⋯",
    yuewen_to_merge=['挨过几多苦头'],
    yuewen_merged="挨过几多苦头⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="才可以练成这举世无双的脚瓜？",
    yuewen_to_merge=['先至可以练成呢只举世无双嘅脚瓜'],
    yuewen_merged="先至可以练成呢只举世无双嘅脚瓜？",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_37
# noinspection PyArgumentList
merging_test_cases_block_38 = [
MergingTestCase(
    zhongwen="我⋯我一定要练成这脚瓜！",
    yuewen_to_merge=['我', '我一定要练成呢只脚挂'],
    yuewen_merged="我⋯我一定要练成呢只脚挂！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="师傅！",
    yuewen_to_merge=['师父'],
    yuewen_merged="师父！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_38
# noinspection PyArgumentList
merging_test_cases_block_39 = [
MergingTestCase(
    zhongwen="我可以去小个便吗？",
    yuewen_to_merge=['我可唔可以去个小便呢'],
    yuewen_merged="我可唔可以去个小便呢？",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_39
# noinspection PyArgumentList
merging_test_cases_block_40 = [
MergingTestCase(
    zhongwen="每次唱这首歌，我都会急小便",
    yuewen_to_merge=['而知点解每一字唱呢首歌都会急小便'],
    yuewen_merged="而知点解每一字唱呢首歌都会急小便",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="现在先去，一回恐怕还是会急",
    yuewen_to_merge=['仅次去定先都怕一阵会再急过'],
    yuewen_merged="仅次去定先，都怕一阵会再急过",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我要黎根收我做徒弟！",
    yuewen_to_merge=['我要黎根收我做徒弟'],
    yuewen_merged="我要黎根收我做徒弟！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="歌，是这样唱的⋯",
    yuewen_to_merge=['嗰首歌系噉唱嘅'],
    yuewen_merged="嗰首歌，系噉唱嘅⋯",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_40
# noinspection PyArgumentList
merging_test_cases_block_41 = [
MergingTestCase(
    zhongwen="黎根听完歌以后，表情有点古怪⋯",
    yuewen_to_merge=['黎今听完首歌之后', '啲表情有啲古怪'],
    yuewen_merged="黎今听完首歌之后，啲表情有啲古怪⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我一定要好好把握这机会",
    yuewen_to_merge=['唔', '我一定要好好把握呢个机会'],
    yuewen_merged="唔，我一定要好好把握呢个机会",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="师傅！你收我做徒弟吧！",
    yuewen_to_merge=['师父', '你收我做徒弟啦'],
    yuewen_merged="师父！你收我做徒弟啦！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="你唔收我做徒弟，我一世都这么跪著！",
    yuewen_to_merge=['你收我做徒弟', '我呢一世都跪喺度'],
    yuewen_merged="你收我做徒弟，我呢一世都跪喺度！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="起来呀！",
    yuewen_to_merge=['起身啊'],
    yuewen_merged="起身啊！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="多谢师傅！",
    yuewen_to_merge=['多谢师父'],
    yuewen_merged="多谢师父！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="不⋯我是叫你扶我起来呀！",
    yuewen_to_merge=['唔系啊', '我系叫你扶我起身啊'],
    yuewen_merged="唔系啊⋯我系叫你扶我起身啊！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="不成了！我的脚瓜太痹了！",
    yuewen_to_merge=['顶唔顺啊', '我个腿挂好鼻啊', '字幕由', 'Amara.org', '社群提供'],
    yuewen_merged="顶唔顺啊！我个腿挂好鼻啊！字幕由Amara.org社群提供",
    difficulty=2,
    verified=True,
),
]  # merging_test_cases_block_41
# noinspection PyArgumentList
merging_test_cases_block_42 = [
MergingTestCase(
    zhongwen="晚饭时，妈妈倒了三杯米酒",
    yuewen_to_merge=['晚饭时候', '妈妈倒咗三杯米酒'],
    yuewen_merged="晚饭时候，妈妈倒咗三杯米酒",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="又将几只橙和蒸好的鸡放到祖先前",
    yuewen_to_merge=['再将几个铲同埋蒸熟咗嘅鸡', '放喺祖先前面'],
    yuewen_merged="再将几个铲同埋蒸熟咗嘅鸡放喺祖先前面",
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈叫我跪低，向祖先请请",
    yuewen_to_merge=['妈妈叫我跪低', '同祖先请请'],
    yuewen_merged="妈妈叫我跪低，同祖先请请",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈跟著又念念有词的",
    yuewen_to_merge=['跟住', '妈妈口噏噏噉讲咗一啲嘢'],
    yuewen_merged="跟住，妈妈口噏噏噉讲咗一啲嘢",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="庄重而温柔地跟我说：",
    yuewen_to_merge=['庄重又温柔紧同我讲'],
    yuewen_merged="庄重又温柔紧同我讲：",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="跟师傅学习，光宗耀祖！",
    yuewen_to_merge=['跟师父学嘢', '当中要祖'],
    yuewen_merged="跟师父学嘢，当中要祖！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_42
# noinspection PyArgumentList
merging_test_cases_block_43 = [
MergingTestCase(
    zhongwen="妈妈在长洲找了间酒楼摆拜师宴",
    yuewen_to_merge=['妈妈喺长洲揾咗间酒楼', '摆咗几回白丝宴'],
    yuewen_merged="妈妈喺长洲揾咗间酒楼摆咗几回白丝宴",
    verified=True,
),
MergingTestCase(
    zhongwen="珊珊因为去了集训，没来",
    yuewen_to_merge=['但系山神就去咗集训', '冇嚟到'],
    yuewen_merged="但系山神就去咗集训，冇嚟到",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="麦唛，菇时跟得巴都来了",
    yuewen_to_merge=['默默', '姑侍同德巴都嚟咗'],
    yuewen_merged="默默，姑侍同德巴都嚟咗",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="还带著成绩表，奖牌和大包",
    yuewen_to_merge=['仲带埋成绩表', '奖牌', '同大包嚟添'],
    yuewen_merged="仲带埋成绩表，奖牌同大包嚟添",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="吃过鸡丝翅，就是拜师仪式",
    yuewen_to_merge=['食完鸡丝翅', '就到咗拜师仪式'],
    yuewen_merged="食完鸡丝翅，就到咗拜师仪式",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="妈妈倒了杯茶给我，让我给师傅喝",
    yuewen_to_merge=['妈妈针咗杯热茶畀我', '叫我弟畀师父饮'],
    yuewen_merged="妈妈针咗杯热茶畀我，叫我弟畀师父饮",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我千辛万苦来长洲找黎根⋯",
    yuewen_to_merge=['哦', '我前三万苦嚟到长洲揾嚟近'],
    yuewen_merged="哦，我前三万苦嚟到长洲揾嚟近⋯",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="我终于可以跟珊珊一起练滑浪风帆了！",
    yuewen_to_merge=['哦', '我终于可以同山神一齐练习玩弄风范啊'],
    yuewen_merged="哦，我终于可以同山神一齐练习玩弄风范啊！",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="我将茶递给黎根，黎根他⋯",
    yuewen_to_merge=['我将杯茶递咗畀嚟跟嚟跟佢', '亦唔系'],
    yuewen_merged="我将杯茶递咗畀嚟跟，嚟跟佢⋯亦唔系",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="师傅他把茶喝了，正式收我做徒弟",
    yuewen_to_merge=['师父佢饮咗杯茶', '正式收咗我做徒弟嘞'],
    yuewen_merged="师父佢饮咗杯茶，正式收咗我做徒弟嘞",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="长洲的乡绅父老拍掌拍得特别落力",
    yuewen_to_merge=['特别系长洲啲乡亲父老', '拍奖拍得特别落力'],
    yuewen_merged="特别系长洲啲乡亲父老拍奖拍得特别落力",
    verified=True,
),
MergingTestCase(
    zhongwen="多谢各位赏面！多谢各位！",
    yuewen_to_merge=['多谢各位上面', '多谢各位'],
    yuewen_merged="多谢各位上面！多谢各位！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="第一项，是滑浪风帆！",
    yuewen_to_merge=['第一样系滑浪风范', '呢样早已'],
    yuewen_merged="第一样，系滑浪风范！呢样早已",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="另一项绝技，我打算传给这个新徒弟⋯",
    yuewen_to_merge=['另一项绝技我打算传畀呢个新修嘅徒弟'],
    yuewen_merged="另一项绝技，我打算传畀呢个新修嘅徒弟⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="发扬光大！",
    yuewen_to_merge=['发扬光大'],
    yuewen_merged="发扬光大！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="请问那是什么绝技呢？",
    yuewen_to_merge=['噉请问嗰样绝技系乜嘢啊'],
    yuewen_merged="噉请问嗰样绝技系乜嘢啊？",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_43
# noinspection PyArgumentList
merging_test_cases_block_44 = [
MergingTestCase(
    zhongwen="第二项绝技，就是⋯",
    yuewen_to_merge=['第二样绝技就系'],
    yuewen_merged="第二样绝技，就系⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="抢包山！",
    yuewen_to_merge=['抢爆山'],
    yuewen_merged="抢爆山！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_44
# noinspection PyArgumentList
merging_test_cases_block_45 = [
MergingTestCase(
    zhongwen="抢包山？",
    yuewen_to_merge=['抢包山?'],
    yuewen_merged="抢包山？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="年轻观众可能不知「抢包山」何物",
    yuewen_to_merge=['年轻嘅观众可能唔知乜嘢系抢包山呀'],
    yuewen_merged="年轻嘅观众可能唔知乜嘢系「抢包山」呀",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="什么是包山呢？",
    yuewen_to_merge=['噉乜嘢系包山呢?'],
    yuewen_merged="噉乜嘢系包山呢？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="顾名思义⋯",
    yuewen_to_merge=['顾名思义'],
    yuewen_merged="顾名思义⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="包山就是一座由好多好多包砌起的山！",
    yuewen_to_merge=['包山就系一座由好多', '好多', '好多包砌起嘅山'],
    yuewen_merged="包山就系一座由好多好多好多包砌起嘅山！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="一座包山，起码六、七层楼高⋯",
    yuewen_to_merge=['一座包山起码六七层楼高'],
    yuewen_merged="一座包山，起码六、七层楼高⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="你可以想像一下包山有多高了吧？",
    yuewen_to_merge=['噉你可以想像一下嗰度有几多包喇'],
    yuewen_merged="噉你可以想像一下嗰度有几多包喇？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="抢包山，就是要把包山上的包抢到手！",
    yuewen_to_merge=['噉抢包山', '自然就系要将包山嘅包抢到手'],
    yuewen_merged="噉抢包山，自然就系要将包山嘅包抢到手！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="抢得位置愈高的包，就是愈大的祝福",
    yuewen_to_merge=['抢到位置越高嘅包', '就代表越大嘅祝福'],
    yuewen_merged="抢到位置越高嘅包，就代表越大嘅祝福",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="在1978年两座包山忽然倒下，多人重伤",
    yuewen_to_merge=['但喺1978年', '两座包山突然塌咗', '都去抢包山'],
    yuewen_merged="但喺1978年两座包山突然塌咗，都去抢包山",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="「抢包山」从此被禁！",
    yuewen_to_merge=['而长洲特有嘅传统亦占备为榜'],
    yuewen_merged="而长洲特有嘅传统亦占备为榜！",
    difficulty=2,
    verified=True,
),
MergingTestCase(
    zhongwen="而长洲独有的传统，亦渐被遗忘",
    yuewen_to_merge=['长洲特有嘅传统亦占备为榜'],
    yuewen_merged="长洲特有嘅传统，亦占备为榜",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_45
# noinspection PyArgumentList
merging_test_cases_block_46 = [
MergingTestCase(
    zhongwen="奥运金牌⋯这一世是没有机会的了",
    yuewen_to_merge=['奥运金牌', '我谂呢一世都唔会攞到'],
    yuewen_merged="奥运金牌⋯我谂呢一世都唔会攞到",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="每个星期六我都搭船过长洲",
    yuewen_to_merge=['每个礼拜六', '我都会搭船过长洲'],
    yuewen_merged="每个礼拜六我都会搭船过长洲",
    verified=True,
),
MergingTestCase(
    zhongwen="去学抢包山⋯",
    yuewen_to_merge=['去学抢包山'],
    yuewen_merged="去学抢包山⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="一项没有奖牌，没有对手，没有比赛⋯",
    yuewen_to_merge=['一日冇奖牌', '冇对手', '冇比赛'],
    yuewen_merged="一日冇奖牌，冇对手，冇比赛⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="更坏的是，连包山也没有！",
    yuewen_to_merge=['更衰嘅系', '连包山都冇'],
    yuewen_merged="更衰嘅系，连包山都冇！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="师傅只是叫我去他的家⋯",
    yuewen_to_merge=['师傅净系叫我去佢屋企'],
    yuewen_merged="师傅净系叫我去佢屋企⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="碰！三番！",
    yuewen_to_merge=['通', '三番'],
    yuewen_merged="通！三番！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="别躲懒！继续练！",
    yuewen_to_merge=['冇偷懒', '继续练'],
    yuewen_merged="冇偷懒！继续练！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="一天，珊珊到了师传家！",
    yuewen_to_merge=['有一日', '山伞嚟咗师傅屋企'],
    yuewen_merged="有一日，山伞嚟咗师傅屋企！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="珊珊！我的师姐珊珊！",
    yuewen_to_merge=['山伞', '我个师仔山伞啊'],
    yuewen_merged="山伞！我个师仔山伞啊！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="可以看见珊珊⋯",
    yuewen_to_merge=['可以见到山伞'],
    yuewen_merged="可以见到山伞⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="这几个星期爬得再辛苦也是值得的！",
    yuewen_to_merge=['爬得咁辛苦', '都系值得㗎'],
    yuewen_merged="爬得咁辛苦都系值得㗎！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="珊珊！",
    yuewen_to_merge=['山伞'],
    yuewen_merged="山伞！",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="珊你个头！继续练习！",
    yuewen_to_merge=['伞你个头啊', '继续练习'],
    yuewen_merged="伞你个头啊！继续练习！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_46
# noinspection PyArgumentList
merging_test_cases_block_47 = [
MergingTestCase(
    zhongwen="还不去？",
    yuewen_to_merge=['仲唔系'],
    yuewen_merged="仲唔系？",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我咁大个仔，什么「头」也给骂过⋯",
    yuewen_to_merge=['我咁大个仔', '乜嘢头都畀人闹过'],
    yuewen_merged="我咁大个仔，乜嘢「头」都畀人闹过⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="不知道为什么「珊你个头」却特别刺耳",
    yuewen_to_merge=['但系山呢个头唔知点解特别瘾'],
    yuewen_merged="但系「山呢个头」唔知点解特别瘾",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我⋯我⋯",
    yuewen_to_merge=['我', '我'],
    yuewen_merged="我⋯我⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="我唔学抢包山了！",
    yuewen_to_merge=['我唔好抢包纱嘞', '字幕由纱友提供'],
    yuewen_merged="我唔好抢包纱嘞！字幕由纱友提供",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_47
# noinspection PyArgumentList
merging_test_cases_block_48 = [
MergingTestCase(
    zhongwen="鸡尾包！新鲜出炉！",
    yuewen_to_merge=['鸡尾包', '啱啱出炉嘅'],
    yuewen_merged="鸡尾包！啱啱出炉嘅！",
    difficulty=1,
    verified=True,
),
]  # merging_test_cases_block_48
# noinspection PyArgumentList
merging_test_cases_block_49 = [
MergingTestCase(
    zhongwen="其实鸡尾包呢⋯",
    yuewen_to_merge=['其实鸡尾爆呢'],
    yuewen_merged="其实鸡尾爆呢⋯",
    difficulty=1,
    verified=True,
),
MergingTestCase(
    zhongwen="你说这似不似鸡尾？",
    yuewen_to_merge=['吓', '你话噉样似唔似鸡尾呀', '哈哈哈哈'],
    yuewen_merged="吓，你话噉样似唔似鸡尾呀？哈哈哈哈",
    difficulty=2,
    verified=True,
),
]  # merging_test_cases_block_49
# noinspection PyArgumentList
merging_test_cases_block_50 = [
    MergingTestCase(
        zhongwen="麦兜他学东西⋯还可以",
        yuewen_to_merge=["麦兜嘅学嘢呢都仲可以"],
        yuewen_merged="麦兜嘅学嘢呢⋯都仲可以",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="黎根接著说了一大堆话⋯",
        yuewen_to_merge=["跟住黎根讲咗一大堆说话"],
        yuewen_merged="跟住黎根讲咗一大堆说话⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="他的抱负，他对麦兜的期望",
        yuewen_to_merge=["讲下佢嘅抱负", "佢对麦兜嘅期望"],
        yuewen_merged="讲下佢嘅抱负，佢对麦兜嘅期望",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="黎根越说越兴奋，直到双眼发光",
        yuewen_to_merge=["黎根越讲越兴奋"],
        yuewen_merged="黎根越讲越兴奋",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="神功戏和现代器械操",
        yuewen_to_merge=["神功气", "现代气蟹粗"],
        yuewen_merged="神功气现代气蟹粗",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="缩脚，唔该！",
        yuewen_to_merge=["缩嗰只脚唔该"],
        yuewen_merged="缩嗰只脚，唔该！",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="你看！",
        yuewen_to_merge=["你睇下"],
        yuewen_merged="你睇下！",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="这脚瓜⋯好粗好大！比一节瓜还要大！",
        yuewen_to_merge=["哗", "呢节", "呢节脚瓜好粗好大呀", "仲大过节瓜"],
        yuewen_merged="哗，呢节⋯呢节脚瓜好粗好大呀！仲大过节瓜！",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="脚瓜的肌肉非常结实⋯",
        yuewen_to_merge=["脚瓜嘅肌肉非常结实"],
        yuewen_merged="脚瓜嘅肌肉非常结实⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="青筋凸现，钢线似的",
        yuewen_to_merge=["啲青筋凸晒出嚟", "好似钢线噉"],
        yuewen_merged="啲青筋凸晒出嚟，好似钢线噉",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="脚趾甲有一寸厚，究竟⋯",
        yuewen_to_merge=["脚趾弓啲脚夹成串咁厚", "究竟"],
        yuewen_merged="脚趾弓啲脚夹成串咁厚，究竟⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="要走过几多座山",
        yuewen_to_merge=["要行个几度呢?", "几多座山"],
        yuewen_merged="要行个几度呢?几多座山?",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="才可以练成这举世无双的脚瓜？",
        yuewen_to_merge=["先至可以练成呢一只举细无伤嘅脚瓜"],
        yuewen_merged="先至可以练成呢一只举细无伤嘅脚瓜？",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_50
# noinspection PyArgumentList
merging_test_cases_block_51 = [
    MergingTestCase(
        zhongwen="我个仔⋯",
        yuewen_to_merge=["我个仔"],
        yuewen_merged="我个仔⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="你个仔，他日都会有这只大脚瓜",
        yuewen_to_merge=["你个仔", "第时都会有我咁大只脚瓜"],
        yuewen_merged="你个仔，第时都会有我咁大只脚瓜",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="其实我也不知道个仔要这么粗的脚瓜⋯",
        yuewen_to_merge=["其实", "我都唔知我仔要咁粗嘅脚瓜"],
        yuewen_merged="其实我都唔知我仔要咁粗嘅脚瓜⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="可是看见那些凸现的青筋，不知怎样⋯",
        yuewen_to_merge=["但系见到佢一条条凸起嘅青筋", "唔知点解"],
        yuewen_merged="但系见到佢一条条凸起嘅青筋，唔知点解⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我想起麦兜的爸爸，阿炳",
        yuewen_to_merge=["我", "我谂起麦兜嘅爸爸", "阿炳"],
        yuewen_merged="我⋯我谂起麦兜嘅爸爸，阿炳",
        difficulty=2,
        verified=True,
    ),
]  # merging_test_cases_block_51
# noinspection PyArgumentList
merging_test_cases_block_52 = [
    MergingTestCase(
        zhongwen="我找来找去也找不到那部电子英文辞典",
        yuewen_to_merge=["我揾完成间屋", "都揾唔到部电子英文词典"],
        yuewen_merged="我揾完成间屋都揾唔到部电子英文词典",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="跑哪去了？",
        yuewen_to_merge=["去咗边呢"],
        yuewen_merged="去咗边呢？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="难道⋯不会吧？",
        yuewen_to_merge=["唔通", "冇理由㗎"],
        yuewen_merged="唔通⋯冇理由㗎？",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_52
# noinspection PyArgumentList
merging_test_cases_block_53 = [
    MergingTestCase(
        zhongwen="想不到真的让妈妈拿去了。吓得我！",
        yuewen_to_merge=["咦", "估唔到真系妈妈攞咗㖞", "吓得我啊"],
        yuewen_merged="咦，估唔到真系妈妈攞咗㖞。吓得我啊！",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="妈妈怎么会写起英文信？",
        yuewen_to_merge=["点解妈妈会用英文写信嘅"],
        yuewen_merged="点解妈妈会用英文写信嘅？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我猜是妈妈用电子辞典逐个字译成英文",
        yuewen_to_merge=["我谂妈妈佢系好辛苦用电子词典", "逐个逐个字译做英文"],
        yuewen_merged="我谂妈妈佢系好辛苦用电子词典逐个逐个字译做英文",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="于是我又用电子辞典把信译回中文",
        yuewen_to_merge=["于是我让返电子词典", "将封信译返做中文"],
        yuewen_merged="于是我让返电子词典将封信译返做中文",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="信，是妈妈写给奥委会主席的",
        yuewen_to_merge=["封信原来系妈妈写畀奥委会主席㗎"],
        yuewen_merged="封信，原来系妈妈写畀奥委会主席㗎",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「亲爱的主席：」",
        yuewen_to_merge=["亲爱的主席"],
        yuewen_merged="「亲爱的主席：」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「你好吗？我很好！」",
        yuewen_to_merge=["你好吗", "我很好"],
        yuewen_merged="「你好吗？我很好！」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「你吃包吗？我吃包！」",
        yuewen_to_merge=["你吃包吗", "我吃包"],
        yuewen_merged="「你吃包吗？我吃包！」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「我们居住在香港这里的人，很爱吃包」",
        yuewen_to_merge=["我门居住在香港这类的人", "肯爱吃包"],
        yuewen_merged="「我门居住在香港这类的人，肯爱吃包」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「小笼包，上海包，广东包，莲蓉包」",
        yuewen_to_merge=["小笼包", "上海包", "广东包", "联融包"],
        yuewen_merged="「小笼包，上海包，广东包，联融包」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「好朋友，我认为",
        yuewen_to_merge=["好朋友", "我认为"],
        yuewen_merged="「好朋友，我认为",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="抢劫那些包，十分重要」",
        yuewen_to_merge=["抢劫嗰些包十分重要"],
        yuewen_merged="抢劫嗰些包，十分重要」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「也算是运动，就真！」",
        yuewen_to_merge=["也算是运动就真"],
        yuewen_merged="「也算是运动，就真！」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「要大力！大吃晚上的粥，和大节瓜！」",
        yuewen_to_merge=["要大力", "大吃", "吻上的粥", "和大字瓜"],
        yuewen_merged="「要大力！大吃吻上的粥，和大字瓜！」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「按照我愚蠢的见解⋯」",
        yuewen_to_merge=["按照我愚蠢的见解"],
        yuewen_merged="「按照我愚蠢的见解⋯」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「抢劫那些包，是奥运会比赛」",
        yuewen_to_merge=["抢劫嗰些包", "系奥运会比赛"],
        yuewen_merged="「抢劫嗰些包，系奥运会比赛」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「让全世界的体育家，抢过！」",
        yuewen_to_merge=["让全世界嘅体育家抢过"],
        yuewen_merged="「让全世界嘅体育家，抢过！」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「世界便和平！」",
        yuewen_to_merge=["世界变和平"],
        yuewen_merged="「世界变和平！」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「你有孩子吗？」",
        yuewen_to_merge=["你有孩子吗"],
        yuewen_merged="「你有孩子吗？」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「我有一个孩子，麦兜」",
        yuewen_to_merge=["我有一个孩子", "麦兜"],
        yuewen_merged="「我有一个孩子，麦兜」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="终于讲到我了！",
        yuewen_to_merge=["终于讲到我啦"],
        yuewen_merged="终于讲到我啦！",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「他是一个好男孩」",
        yuewen_to_merge=["她系一个好男孩"],
        yuewen_merged="「她系一个好男孩」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「他非常懂得抢劫那些包」",
        yuewen_to_merge=["她非常懂得抢劫嗰些包"],
        yuewen_merged="「她非常懂得抢劫嗰些包」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「有一天，我看见他，抢劫包⋯」",
        yuewen_to_merge=["有一天我看见她抢劫包"],
        yuewen_merged="「有一天，我看见她抢劫包⋯」",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「抢了一个奥运金牌」",
        yuewen_to_merge=["抢了一个奥运金牌"],
        yuewen_merged="「抢了一个奥运金牌」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「那便是一个母亲能够有的最大的安慰」",
        yuewen_to_merge=["哪便是一个母亲能够有的最好的", "最大的安慰"],
        yuewen_merged="「哪便是一个母亲能够有的最好的最大的安慰」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「孩子的才干，得到了世界人类的知道」",
        yuewen_to_merge=["孩子的才干得到了世界人类的知道"],
        yuewen_merged="「孩子的才干，得到了世界人类的知道」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「父母愿意做什么的东西都得」",
        yuewen_to_merge=["父母愿意做什么的东西都得"],
        yuewen_merged="「父母愿意做什么的东西都得」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「于是我写了这忽然间的信给你」",
        yuewen_to_merge=["于是我写了这忽然间的信给你"],
        yuewen_merged="「于是我写了这忽然间的信给你」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「虽然你不知道我是什么微细的东西」",
        yuewen_to_merge=["虽然你不知道我是什么微细的东西"],
        yuewen_merged="「虽然你不知道我是什么微细的东西」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「但我的孩子很大，很大！」",
        yuewen_to_merge=["但我的孩子很大很大"],
        yuewen_merged="「但我的孩子很大，很大！」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「有一天，你都会知道」",
        yuewen_to_merge=["有一天你都会知道"],
        yuewen_merged="「有一天，你都会知道」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「多谢合作！」",
        yuewen_to_merge=["多谢合作"],
        yuewen_merged="「多谢合作！」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「你忠实的，麦太」",
        yuewen_to_merge=["你忠实的", "麦太"],
        yuewen_merged="「你忠实的，麦太」",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_53
# noinspection PyArgumentList
merging_test_cases_block_54 = [
    MergingTestCase(
        zhongwen="可是，我依然努力练习抢包山",
        yuewen_to_merge=["但系", "我依然努力练习抢包生"],
        yuewen_merged="但系，我依然努力练习抢包生",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="因为，我爱我妈妈",
        yuewen_to_merge=["因为我爱我妈妈"],
        yuewen_merged="因为，我爱我妈妈",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="师傅说我攀爬功夫已经不错",
        yuewen_to_merge=["师傅话", "我嘅攀爬功夫已经唔错"],
        yuewen_merged="师傅话我嘅攀爬功夫已经唔错",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="可以开始教我「十二路抢包手」",
        yuewen_to_merge=["可以开始教我十二路抢包手"],
        yuewen_merged="可以开始教我「十二路抢包手」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="师傅说当年师祖耍出这套",
        yuewen_to_merge=["师傅话", "当年师祖使出呢套"],
        yuewen_merged="师傅话当年师祖使出呢套",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「十二路抢包手」⋯",
        yuewen_to_merge=["十二路抢包手"],
        yuewen_merged="「十二路抢包手」⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="后来麦唛告诉我⋯",
        yuewen_to_merge=["后来", "默默话我知"],
        yuewen_merged="后来默默话我知⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="林世荣即是猪肉荣，是黄飞鸿的徒弟",
        yuewen_to_merge=["林世荣即系猪肉荣", "系黄飞鸿嘅徒弟"],
        yuewen_merged="林世荣即系猪肉荣，系黄飞鸿嘅徒弟",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我一边练习，一边胡思乱想；始终⋯",
        yuewen_to_merge=["我一边练习", "一边乱练", "一边谂嘢", "始终"],
        yuewen_merged="我一边练习，一边乱练，一边谂嘢；始终⋯",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="于是我咬实牙根⋯",
        yuewen_to_merge=["于是我咬细牙根"],
        yuewen_merged="于是我咬细牙根⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="一步一步，一爪一爪⋯",
        yuewen_to_merge=["一步一步", "一爪一爪"],
        yuewen_merged="一步一步，一爪一爪⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我最后终于练成「十二路抢包手」",
        yuewen_to_merge=["最后", "我终于练成十二路抢包手啦"],
        yuewen_merged="最后我终于练成「十二路抢包手」啦",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_54
# noinspection PyArgumentList
merging_test_cases_block_55 = [
    MergingTestCase(
        zhongwen="喂，我是麦兜",
        yuewen_to_merge=["喂", "我系麦兜啊"],
        yuewen_merged="喂，我系麦兜啊",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="刚才的是小朋友麦兜，我是大个佬麦兜",
        yuewen_to_merge=["正话嗰个系细路仔麦兜", "我系大个佬麦兜"],
        yuewen_merged="正话嗰个系细路仔麦兜，我系大个佬麦兜",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="小朋友麦兜和大个佬麦兜除了声音不同⋯",
        yuewen_to_merge=["细路仔麦兜同大个佬麦兜除咗把声唔同之外"],
        yuewen_merged="细路仔麦兜同大个佬麦兜除咗把声唔同之外⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="希望⋯失望⋯",
        yuewen_to_merge=["希望", "失望"],
        yuewen_merged="希望⋯失望⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="希望⋯",
        yuewen_to_merge=["希望"],
        yuewen_merged="希望⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="久而久之，就变成大个佬麦兜",
        yuewen_to_merge=["搞咗一轮", "就变咗大个佬麦兜"],
        yuewen_merged="搞咗一轮，就变咗大个佬麦兜",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我现在还是多说点小朋友麦兜",
        yuewen_to_merge=["不过", "而家我都系想讲返细路仔麦兜"],
        yuewen_merged="不过，而家我都系想讲返细路仔麦兜",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="小朋友麦兜仍然希望希望⋯",
        yuewen_to_merge=["细路仔麦兜仲系希望希望"],
        yuewen_merged="细路仔麦兜仲系希望希望⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="对，那时我还没吃过火鸡",
        yuewen_to_merge=["系啊", "我嗰阵", "我真系仲未食过火鸡"],
        yuewen_merged="系啊，我嗰阵我真系仲未食过火鸡",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="关于火鸡的一切⋯",
        yuewen_to_merge=["所有关于火鸡嘅嘢"],
        yuewen_merged="所有关于火鸡嘅嘢⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="一片片比外边的雪还要白的鸡胸肉⋯",
        yuewen_to_merge=["一片一片", "比窗外面嘅雪仲要白嘅鸡胸肉"],
        yuewen_merged="一片一片比窗外面嘅雪仲要白嘅鸡胸肉⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="香气直入灵魂⋯",
        yuewen_to_merge=["香气直入灵魂"],
        yuewen_merged="香气直入灵魂⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="围住这香而圣洁的肉⋯",
        yuewen_to_merge=["围住呢一嚿好香好香又好盛洁嘅肉"],
        yuewen_merged="围住呢一嚿好香好香又好盛洁嘅肉⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="在圣诞夜中飞呀，飞⋯",
        yuewen_to_merge=["喺圣诞夜里面", "飞呀", "飞呀"],
        yuewen_merged="喺圣诞夜里面飞呀，飞呀⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="这关于火鸡的一切，不过是我的想像",
        yuewen_to_merge=["但系呢一切一切关于火鸡嘅嘢", "都不过系我嘅想像"],
        yuewen_merged="但系呢一切一切关于火鸡嘅嘢，都不过系我嘅想像",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我从来没吃过火鸡⋯",
        yuewen_to_merge=["因为我从来都未食过火鸡"],
        yuewen_merged="因为我从来都未食过火鸡⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我们一家两口，吃不下",
        yuewen_to_merge=["我哋一家两口点食都食唔晒"],
        yuewen_merged="我哋一家两口，点食都食唔晒",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="有年圣诞节妈妈买了半只烧鸭庆祝",
        yuewen_to_merge=["有一年圣诞节", "妈妈买咗半边烧鸭庆祝"],
        yuewen_merged="有一年圣诞节妈妈买咗半边烧鸭庆祝",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="当时的我，十分十分失望",
        yuewen_to_merge=["当时我真系十分十分之失望"],
        yuewen_merged="当时我，真系十分十分之失望",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="又有一年，一间百货公司结业",
        yuewen_to_merge=["又有一年", "有间大薄货公司倒闭"],
        yuewen_merged="又有一年，有间大薄货公司倒闭",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="那日妈妈竟然跟我说⋯",
        yuewen_to_merge=["嗰日妈妈竟然同我讲", "佢话"],
        yuewen_merged="嗰日妈妈竟然同我讲⋯佢话⋯",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我跟妈妈把火鸡揪回家的路上⋯",
        yuewen_to_merge=["我同妈妈抽住只火鸡行返屋企嗰阵", "喺嗰阵时"],
        yuewen_merged="我同妈妈抽住只火鸡行返屋企嗰阵，喺嗰阵时⋯",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我学著妈妈，把双手涂满盐⋯",
        yuewen_to_merge=["我同妈妈噉双手查满盐"],
        yuewen_merged="我同妈妈，噉双手查满盐⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="在火鸡丰厚的鸡胸上擦呀，擦",
        yuewen_to_merge=["喺火鸡封口嘅鸡胸度", "起细噉啫啫"],
        yuewen_merged="喺火鸡封口嘅鸡胸度，起细噉啫啫",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="联火鸡时⋯",
        yuewen_to_merge=["联火鸡时"],
        yuewen_merged="联火鸡时⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="妈妈不留神漏出了火鸡内的洋葱粒",
        yuewen_to_merge=["妈妈一个唔觉意", "畀酿喺火鸡里面嘅火鸡内脏洋葱粒"],
        yuewen_merged="妈妈一个唔觉意畀酿喺火鸡里面嘅火鸡内脏洋葱粒",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我说：火鸡「疴烂煮」！",
        yuewen_to_merge=["我话", "火鸡我能住呀"],
        yuewen_merged="我话：火鸡「我能住」呀！",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="焗炉戚戚恻恻，戚戚恻恻⋯",
        yuewen_to_merge=["个焗炉叱叱叱叱", "叱叱叱咁"],
        yuewen_merged="个焗炉叱叱叱叱，叱叱叱咁⋯",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_55
# noinspection PyArgumentList
merging_test_cases_block_56 = [
    MergingTestCase(
        zhongwen="好靓的晚上啊！",
        yuewen_to_merge=["好靓嘅夜晚呀"],
        yuewen_merged="好靓嘅夜晚呀！",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="点点灯光在海面走来走去⋯",
        yuewen_to_merge=["点点点点嘅灯光喺海上面走来走去"],
        yuewen_merged="点点点点嘅灯光喺海上面走来走去⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="美丽又温柔",
        yuewen_to_merge=["又靓", "又温柔"],
        yuewen_merged="又靓又温柔",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="真的好靓！",
        yuewen_to_merge=["真系好靓"],
        yuewen_merged="真系好靓！",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_56
# noinspection PyArgumentList
merging_test_cases_block_57 = [
    MergingTestCase(
        zhongwen="甚至杯面，烧鸭的味道也没有这么浓",
        yuewen_to_merge=["连烧鸭连杯面都冇咁浓嘅味道"],
        yuewen_merged="连烧鸭连杯面都冇咁浓嘅味道",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="火鸡的味道把我每一个味蕾缠住⋯",
        yuewen_to_merge=["火鸡嘅味道喺我嘅每一个味蕾度缠住"],
        yuewen_merged="火鸡嘅味道喺我嘅每一个味蕾度缠住⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="爆发⋯缠住⋯爆发⋯",
        yuewen_to_merge=["爆发", "缠住", "爆发"],
        yuewen_merged="爆发⋯缠住⋯爆发⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="最靓最靓，最犀利，而且最温柔",
        yuewen_to_merge=["最靓", "最靓", "最犀利", "亦都系最温柔"],
        yuewen_merged="最靓最靓，最犀利，亦都系最温柔",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_57
# noinspection PyArgumentList
merging_test_cases_block_58 = [
    MergingTestCase(
        zhongwen="第二天我睡得很晏⋯",
        yuewen_to_merge=["第二日我瞓到好硬"],
        yuewen_merged="第二日我瞓到好硬⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="刷过牙我还感觉到火鸡的美味",
        yuewen_to_merge=["测完牙", "我仲感觉到火鸡嘅美味"],
        yuewen_merged="测完牙我仲感觉到火鸡嘅美味",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="因为早餐吃得晚⋯",
        yuewen_to_merge=["因为早餐食得硬"],
        yuewen_merged="因为早餐食得硬⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="不用说，那夜就是我渴望了⋯",
        yuewen_to_merge=["嗰晚唔使讲", "当然系食我限咗"],
        yuewen_merged="嗰晚唔使讲，当然系食我限咗⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="很久很久很久的⋯圣诞火鸡大餐！",
        yuewen_to_merge=["好耐好耐好耐好耐嘅圣诞火鸡大餐"],
        yuewen_merged="好耐好耐好耐好耐嘅⋯圣诞火鸡大餐！",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="一片片的火鸡肉和伴碟的薯仔和节瓜⋯",
        yuewen_to_merge=["一片一片嘅火鸡肉", "半碟嘅有薯仔同节瓜"],
        yuewen_merged="一片一片嘅火鸡肉半碟嘅有薯仔同节瓜⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我们真的好兴奋，好满足",
        yuewen_to_merge=["我哋真系好兴奋", "好满足"],
        yuewen_merged="我哋真系好兴奋，好满足",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="之后，我们吃了一个星期的⋯",
        yuewen_to_merge=["之后我哋仲食咗一个礼拜嘅"],
        yuewen_merged="之后，我哋仲食咗一个礼拜嘅⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我大著胆跟妈妈说：不如去饮茶吖",
        yuewen_to_merge=["我嘅记心肝同妈妈讲", "不如饮茶"],
        yuewen_merged="我嘅记心肝同妈妈讲：不如饮茶",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="妈妈骂我「冇衣食」⋯",
        yuewen_to_merge=["妈妈闹我冇意食"],
        yuewen_merged="妈妈闹我「冇意食」⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="之后，妈妈又有计⋯",
        yuewen_to_merge=["之后妈妈又有计"],
        yuewen_merged="之后，妈妈又有计⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="她把冰箱内剩下来的火鸡肉撕呀撕",
        yuewen_to_merge=["佢将雪柜净返嘅火鸡肉", "系噉撕系噉撕"],
        yuewen_merged="佢将雪柜净返嘅火鸡肉系噉撕系噉撕",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="银芽火鸡丝炒米，好味道",
        yuewen_to_merge=["银牙火鸡丝炒米好味道噉"],
        yuewen_merged="银牙火鸡丝炒米，好味道噉",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="唉，我好后悔讲过一句「火鸡疴烂煮」",
        yuewen_to_merge=["唉", "我后悔讲过火鸡阿宁处呢句嘢"],
        yuewen_merged="唉，我后悔讲过「火鸡阿宁处」呢句嘢",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="端午节，当我翻开我最喜欢吃的裹蒸粽⋯",
        yuewen_to_merge=["到端午节", "当我督开我最钟意食嘅果精粽嘅时候"],
        yuewen_merged="到端午节，当我督开我最钟意食嘅果精粽嘅时候⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="发现咸蛋旁边是一件火鸡背的时候⋯",
        yuewen_to_merge=["发现宿喺咸蛋旁边嘅系一件火鸡背脊"],
        yuewen_merged="发现宿喺咸蛋旁边嘅系一件火鸡背脊⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我脑部一时想唔通，哭起来",
        yuewen_to_merge=["我脑部一时想唔通", "喊咗起上嚟"],
        yuewen_merged="我脑部一时想唔通，喊咗起上嚟",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="救命呀！",
        yuewen_to_merge=["救命啊"],
        yuewen_merged="救命啊！",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我的美梦跟恶梦亦同时完结",
        yuewen_to_merge=["我嘅美梦同噩梦", "都同时完结"],
        yuewen_merged="我嘅美梦同噩梦都同时完结",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="后来我才知道⋯",
        yuewen_to_merge=["后来我先知道"],
        yuewen_merged="后来我先知道⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="即是说，火鸡死掉后跟我们一起的日子",
        yuewen_to_merge=["即系话", "只火鸡死咗之后", "同我哋一齐嘅日子"],
        yuewen_merged="即系话，只火鸡死咗之后同我哋一齐嘅日子",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我还发觉，火鸡的味道⋯",
        yuewen_to_merge=["我仲发觉到", "火鸡嘅味道"],
        yuewen_merged="我仲发觉到，火鸡嘅味道⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="将吃未吃和第一口之间已经是最高峰",
        yuewen_to_merge=["味食同食第一啖之间", "已经系佢嘅最高峰"],
        yuewen_merged="味食同食第一啖之间已经系佢嘅最高峰",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="之后的，不过是开始了也就吃下去",
        yuewen_to_merge=["之后", "不过都系食开就食埋落去噉解"],
        yuewen_merged="之后，不过都系食开就食埋落去噉解",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我没有哲学家的头脑⋯",
        yuewen_to_merge=["我冇知学家嘅头脑"],
        yuewen_merged="我冇知学家嘅头脑⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="不知道两件事情应该得出什么道理",
        yuewen_to_merge=["唔知呢两样嘢要得起嘅呢个", "得出啲咩道理"],
        yuewen_merged="唔知呢两样嘢要得起嘅呢个得出啲咩道理",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="可是这些想法⋯",
        yuewen_to_merge=["但系呢啲谂法"],
        yuewen_merged="但系呢啲谂法⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="在我长大后⋯",
        yuewen_to_merge=["喺我长大之后"],
        yuewen_merged="喺我长大之后⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="在一些跟圣诞节无关的日子⋯",
        yuewen_to_merge=["系一啲同圣诞节无关嘅日子"],
        yuewen_merged="系一啲同圣诞节无关嘅日子⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="毫无因由的在我脑中出现过三两次",
        yuewen_to_merge=["无端端噉喺我脑部", "出现过两三次"],
        yuewen_merged="无端端噉喺我脑部出现过两三次",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="一次，是在我自己的婚宴上",
        yuewen_to_merge=["一次", "喺我自己嘅婚宴上"],
        yuewen_merged="一次，喺我自己嘅婚宴上",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="一次⋯",
        yuewen_to_merge=["一次"],
        yuewen_merged="一次⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="那天，我看著天空几缕灰色的烟",
        yuewen_to_merge=["嗰日", "我望住天东几条灰色嘅烟"],
        yuewen_merged="嗰日，我望住天东几条灰色嘅烟",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我好后悔要妈妈扔掉最后几件火鸡",
        yuewen_to_merge=["我后悔", "要妈妈劈咗个忌廉", "火鸡"],
        yuewen_merged="我后悔要妈妈劈咗个忌廉火鸡",
        verified=True,
    ),
]  # merging_test_cases_block_58
# noinspection PyArgumentList
merging_test_cases_block_59 = [
    MergingTestCase(
        zhongwen="向全世界人再次证明⋯",
        yuewen_to_merge=["向全世界人再次证明"],
        yuewen_merged="向全世界人再次证明⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="另方面⋯",
        yuewen_to_merge=["另一方面"],
        yuewen_merged="另一方面⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="香港体运总会霍震霆⋯",
        yuewen_to_merge=["中国香港体育协会企奥委会会长霍振庭"],
        yuewen_merged="中国香港体育协会企奥委会会长霍振庭⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="其中港九新界竹战联谊会⋯",
        yuewen_to_merge=["其中港狗新界足战联谊会"],
        yuewen_merged="其中港狗新界足战联谊会⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="另外，全港茶餐厅员工协会⋯",
        yuewen_to_merge=["另外", "全港茶餐厅联工协会"],
        yuewen_merged="另外，全港茶餐厅联工协会⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="经已发动所有会员⋯",
        yuewen_to_merge=["经热发动所有会员"],
        yuewen_merged="经热发动所有会员⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="争取「掷蛋挞」成为亚运比赛项目",
        yuewen_to_merge=["争取掟蛋挞成为亚运会比赛项目"],
        yuewen_merged="争取「掟蛋挞」成为亚运会比赛项目",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="港九烧味卤味腊味同业会",
        yuewen_to_merge=["港狗烧尾", "掳尾", "立尾同业会"],
        yuewen_merged="港狗烧尾掳尾立尾同业会",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="亦向霍主席当面提出⋯",
        yuewen_to_merge=["亦都向霍主席当面提出"],
        yuewen_merged="亦都向霍主席当面提出⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「挂腊鸭」可以成为亚运比赛项目",
        yuewen_to_merge=["挂立鸭可以成为亚运比赛项目"],
        yuewen_merged="「挂立鸭」可以成为亚运比赛项目",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="较为特别的是，CIC保险营业员联同⋯",
        yuewen_to_merge=["较为特别嘅系", "CIC保险营业员联同"],
        yuewen_merged="较为特别嘅系，CIC保险营业员联同⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="大角咀春田花花幼稚园⋯",
        yuewen_to_merge=["大角嘴春田花花", "幼稚园"],
        yuewen_merged="大角嘴春田花花幼稚园⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="附属小学一班小朋友⋯",
        yuewen_to_merge=["附属小学嘅一班小朋友"],
        yuewen_merged="附属小学嘅一班小朋友⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="争取「抢包山」",
        yuewen_to_merge=["争取抢包山"],
        yuewen_merged="争取「抢包山」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="一项几乎绝迹的运动⋯",
        yuewen_to_merge=["一项几乎绝迹嘅运动"],
        yuewen_merged="一项几乎绝迹嘅运动⋯",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_59
# noinspection PyArgumentList
merging_test_cases_block_60 = [
    MergingTestCase(
        zhongwen="最后⋯",
        yuewen_to_merge=["最后"],
        yuewen_merged="最后⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="最后，一切成烟",
        yuewen_to_merge=["最后全部都系banana"],
        yuewen_merged="最后，全部都系banana",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="最后，他们选了「掷蛋挞」做推介项目",
        yuewen_to_merge=["最后佢哋选咗定蛋挞做推介项目"],
        yuewen_merged="最后，佢哋选咗「定蛋挞」做推介项目",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="至于香港争取申办亚运的口号⋯",
        yuewen_to_merge=["至于香港争取申办亚运嘅口号"],
        yuewen_merged="至于香港争取申办亚运嘅口号⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="亦顺理成章叫成「香港一蛋挞」",
        yuewen_to_merge=["亦都顺理成章噉叫做香港一蛋挞"],
        yuewen_merged="亦都顺理成章噉叫做「香港一蛋挞」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="之后李丽珊蝉联失败⋯",
        yuewen_to_merge=["之后李利山丧乱失败"],
        yuewen_merged="之后李利山丧乱失败⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="亚运主办权⋯",
        yuewen_to_merge=["亚运主办权"],
        yuewen_merged="亚运主办权⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="想著转行当运动员的茶餐厅伙记⋯",
        yuewen_to_merge=["谂住可以转行做运动员嘅茶餐厅伙计"],
        yuewen_merged="谂住可以转行做运动员嘅茶餐厅伙计⋯",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_60
# noinspection PyArgumentList
merging_test_cases_block_61 = [
    MergingTestCase(
        zhongwen="上中学后，我再没有练习抢包手",
        yuewen_to_merge=["上个中学", "我已经再冇练习抢包手"],
        yuewen_merged="上个中学，我已经再冇练习抢包手",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="有时候跟妈妈饮茶⋯",
        yuewen_to_merge=["间中同妈妈饮茶"],
        yuewen_merged="间中同妈妈饮茶⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="之后，茶楼再不卖大包了",
        yuewen_to_merge=["之后茶楼都冇埋大包"],
        yuewen_merged="之后，茶楼都冇埋大包",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_61
# noinspection PyArgumentList
merging_test_cases_block_62 = [
    MergingTestCase(
        zhongwen="每次看见师傅⋯",
        yuewen_to_merge=["每次见到师傅"],
        yuewen_merged="每次见到师傅⋯",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_62
# noinspection PyArgumentList
merging_test_cases_block_63 = [
    MergingTestCase(
        zhongwen="因为环保⋯",
        yuewen_to_merge=["因为环保"],
        yuewen_merged="因为环保⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="师傅说，那阵胶气，相当臭",
        yuewen_to_merge=["师傅话嗰阵胶气都几丑下"],
        yuewen_merged="师傅话，嗰阵胶气，都几丑下",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_63
# noinspection PyArgumentList
merging_test_cases_block_64 = [
    MergingTestCase(
        zhongwen="因为我练过抢包手，身手比较灵活⋯",
        yuewen_to_merge=["因为我练过抢包手", "身手比较灵活"],
        yuewen_merged="因为我练过抢包手，身手比较灵活⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="同学们叫我爬进去看看，说不定会发达",
        yuewen_to_merge=["班同我叫我爬佢睇下", "话唔定会发达"],
        yuewen_merged="班同我叫我爬佢睇下，话唔定会发达",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="于是我就向著这个又黑又窄的洞⋯",
        yuewen_to_merge=["于是我就向住呢一个又黑又窄嘅洞"],
        yuewen_merged="于是我就向住呢一个又黑又窄嘅洞⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="一直爬",
        yuewen_to_merge=["系噉爬", "爬"],
        yuewen_merged="系噉爬爬",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="洞里面什么也没有，只有一个盒",
        yuewen_to_merge=["洞里面乜都冇", "净系有一个盒"],
        yuewen_merged="洞里面乜都冇，净系有一个盒",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我小心揭开盒⋯",
        yuewen_to_merge=["我好小心揭开呢个盒"],
        yuewen_merged="我好小心揭开呢个盒⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="是不是张保仔吃过的呢？",
        yuewen_to_merge=["唔知系咪张宝仔食净㗎啦"],
        yuewen_merged="唔知系咪张宝仔食净㗎啦？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="揸住个包，我忽然明白⋯",
        yuewen_to_merge=["揸住个包", "我忽然明白"],
        yuewen_merged="揸住个包，我忽然明白⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="原来有些事情，没有就是没有",
        yuewen_to_merge=["原来有啲嘢冇", "就真系冇"],
        yuewen_merged="原来有啲嘢，冇就真系冇",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="唔得，就是唔得",
        yuewen_to_merge=["唔得", "就真系唔得"],
        yuewen_merged="唔得，就真系唔得",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="没有鱼蛋没有粗面没去成马尔代夫⋯",
        yuewen_to_merge=["冇鱼蛋", "冇粗面", "冇去买义大夫"],
        yuewen_merged="冇鱼蛋冇粗面冇去买义大夫⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="而张保仔，也没有咬过那个包",
        yuewen_to_merge=["而张宝仔亦都冇咬过个包"],
        yuewen_merged="而张宝仔，亦都冇咬过个包",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="原来蠢，并不那么好笑",
        yuewen_to_merge=["原来", "唔系咁好笑"],
        yuewen_merged="原来，唔系咁好笑",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="蠢会失败⋯",
        yuewen_to_merge=["会失败"],
        yuewen_merged="会失败⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="失望，并不那么好笑",
        yuewen_to_merge=["失望", "唔系咁好笑"],
        yuewen_merged="失望，唔系咁好笑",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="肥，都不一定好笑",
        yuewen_to_merge=["肥", "都未必好笑"],
        yuewen_merged="肥，都未必好笑",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="肥，不一定大力",
        yuewen_to_merge=["肥", "唔一定大力"],
        yuewen_merged="肥，唔一定大力",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="大力，亦不一定得",
        yuewen_to_merge=["大力", "亦都唔一定得"],
        yuewen_merged="大力，亦都唔一定得",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="揸住个包，我忽然想⋯",
        yuewen_to_merge=["揸住个包", "我忽然喺度谂"],
        yuewen_merged="揸住个包，我忽然喺度谂⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="长大了，到我要面对这个实掘掘⋯",
        yuewen_to_merge=["大个咗到我要面对呢一个实角局"],
        yuewen_merged="大个咗，到我要面对呢一个实角局⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="未必可以发梦，未必那么好笑的⋯",
        yuewen_to_merge=["未必到你发梦", "又未必咁好笑嘅"],
        yuewen_merged="未必到你发梦，又未必咁好笑嘅⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="世界的时候，我会怎么样？",
        yuewen_to_merge=["世界嘅时候", "我会系点㗎呢"],
        yuewen_merged="世界嘅时候，我会系点㗎呢？",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_64
# noinspection PyArgumentList
merging_test_cases_block_65 = [
    MergingTestCase(
        zhongwen="「⋯无力挽！」",
        yuewen_to_merge=["无泪弯"],
        yuewen_merged="「⋯无泪弯！」",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_65
# noinspection PyArgumentList
merging_test_cases_block_66 = []  # merging_test_cases_block_66
# noinspection PyArgumentList
merging_test_cases_block_67 = [
    MergingTestCase(
        zhongwen="是的，我就是大个佬麦兜",
        yuewen_to_merge=["系呀", "我就系大个佬麦豆喇"],
        yuewen_merged="系呀，我就系大个佬麦豆喇",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="肥，算大力",
        yuewen_to_merge=["肥啰", "算大力啦"],
        yuewen_merged="肥啰，算大力啦",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="脚瓜是真的大，比一节瓜还要大",
        yuewen_to_merge=["脚瓜真系几大", "仲大过个折瓜"],
        yuewen_merged="脚瓜真系几大，仲大过个折瓜",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="脚瓜上的肌肉非常结实⋯",
        yuewen_to_merge=["脚瓜上面个肌肉非常结实"],
        yuewen_merged="脚瓜上面个肌肉非常结实⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="青筋一条条凸出来，似钢筋",
        yuewen_to_merge=["啲青筋一条一条凸下凸下", "好似钢筋"],
        yuewen_merged="啲青筋一条一条凸下凸下，好似钢筋",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="至于脚趾甲⋯",
        yuewen_to_merge=["至于脚趾弓啲脚甲"],
        yuewen_merged="至于脚趾弓啲脚甲⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="有次我无无聊聊真的量了一下⋯",
        yuewen_to_merge=["有次我无无聊聊真系走去卡下佢"],
        yuewen_merged="有次我无无聊聊真系走去卡下佢⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="足有一寸厚",
        yuewen_to_merge=["哗", "粥粥成串咁厚"],
        yuewen_merged="哗，粥粥成串咁厚",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="是的，故事讲完了",
        yuewen_to_merge=["系呀", "故事讲完喇"],
        yuewen_merged="系呀，故事讲完喇",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="失败⋯尝试⋯",
        yuewen_to_merge=["失败", "尝试"],
        yuewen_merged="失败⋯尝试⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="好多包⋯可是没有包保成功的故事",
        yuewen_to_merge=["好多包", "但系冇包补成功嘅故事"],
        yuewen_merged="好多包⋯但系冇包补成功嘅故事",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="故事说了一轮⋯",
        yuewen_to_merge=["故事讲咗一轮"],
        yuewen_merged="故事讲咗一轮⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="什么也没有？也不是",
        yuewen_to_merge=["乜都冇", "又唔系噃"],
        yuewen_merged="乜都冇？又唔系噃",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="可是楝一双脚瓜站这儿⋯",
        yuewen_to_merge=["但系冻住两个脚瓜企喺度"],
        yuewen_merged="但系冻住两个脚瓜企喺度⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="当浪打过来⋯",
        yuewen_to_merge=["当啲浪打埋嚟"],
        yuewen_merged="当啲浪打埋嚟⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="你知道我麻麻地叻佬，不懂得⋯",
        yuewen_to_merge=["你知我麻麻地叻佬", "唔识得"],
        yuewen_merged="你知我麻麻地叻佬，唔识得⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="替自己的故事加点教训呀锦囊呀那些",
        yuewen_to_merge=["帮自己嘅故事加啲教训呀", "锦囊呀嗰啲嘢"],
        yuewen_merged="帮自己嘅故事加啲教训呀锦囊呀嗰啲嘢",
        verified=True,
    ),
    MergingTestCase(
        zhongwen="可是，浸一双脚瓜站水中⋯",
        yuewen_to_merge=["但系冻住两个脚瓜企喺水嗰度"],
        yuewen_merged="但系，冻住两个脚瓜企喺水嗰度⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="当风吹向我的脑，我会想⋯",
        yuewen_to_merge=["当风", "吹喺我个脑部", "我会谂"],
        yuewen_merged="当风吹喺我个脑部，我会谂⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="如果妈妈看见我这个大脚瓜⋯",
        yuewen_to_merge=["如果妈妈见到我呢个大脚瓜"],
        yuewen_merged="如果妈妈见到我呢个大脚瓜⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="我猜，她会好开心",
        yuewen_to_merge=["我谂佢会好开心"],
        yuewen_merged="我谂，佢会好开心",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_67
# noinspection PyArgumentList
merging_test_cases_block_68 = [
    MergingTestCase(
        zhongwen="不成，还是出个锦囊！",
        yuewen_to_merge=["都系唔好呀", "都系出返个锦囊先得"],
        yuewen_merged="都系唔好呀，都系出返个锦囊先得！",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="妈妈的dot com散掉后，她又有计",
        yuewen_to_merge=["妈妈个Doccom散咗之后", "佢又有计喇"],
        yuewen_merged="妈妈个Doccom散咗之后，佢又有计喇",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="她出版了一本教烹饪的食谱",
        yuewen_to_merge=["佢出咗半教主送嘅食谱", "谂住捞返扎沙"],
        yuewen_merged="佢出咗半教主送嘅食谱，谂住捞返扎沙",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="方法简单，人人可学",
        yuewen_to_merge=["方法简单", "人人都学得识"],
        yuewen_merged="方法简单，人人都学得识",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="「烧鸡」",
        yuewen_to_merge=["烧鸡"],
        yuewen_merged="「烧鸡」",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="材料是⋯鸡",
        yuewen_to_merge=["材料系", "鸡"],
        yuewen_merged="材料系⋯鸡",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="方法：把鸡烧几烧",
        yuewen_to_merge=["方法", "攞只鸡", "去烧佢几烧"],
        yuewen_merged="方法：攞只鸡去烧佢几烧",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="就这样，一味「烧鸡」大功告成",
        yuewen_to_merge=["就噉", "一味烧鸡", "就大功告成喇"],
        yuewen_merged="就噉，一味「烧鸡」就大功告成喇",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="食谱里面补充说：",
        yuewen_to_merge=["食谱度又补充噉话"],
        yuewen_merged="食谱度又补充噉话：",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="如果你想把鸡烧得美味可口⋯",
        yuewen_to_merge=["如果想你个鸡烧得美味可口"],
        yuewen_merged="如果想你个鸡烧得美味可口⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="秘诀是：拜托，把鸡烧好一点⋯",
        yuewen_to_merge=["个秘诀系唔该", "烧得佢好啲啰"],
        yuewen_merged="个秘诀系：唔该，烧得佢好啲啰⋯",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="多谢合作！",
        yuewen_to_merge=["多谢合作"],
        yuewen_merged="多谢合作！",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_68
# noinspection PyArgumentList
merging_test_cases_block_69 = [
    MergingTestCase(
        zhongwen="麻烦你，一客常餐",
        yuewen_to_merge=["唔该", "我要一个常餐啦"],
        yuewen_merged="唔该，我要一个常餐啦",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="常餐？常餐有什么吃？",
        yuewen_to_merge=["常餐", "常餐有咩食㗎"],
        yuewen_merged="常餐？常餐有咩食㗎？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="特餐是什么？",
        yuewen_to_merge=["噉特餐系咩嚟㗎"],
        yuewen_merged="噉特餐系咩嚟㗎？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="快餐又是什么？",
        yuewen_to_merge=["噉快餐又系咩嚟㗎"],
        yuewen_merged="噉快餐又系咩嚟㗎？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="午餐吃什么？",
        yuewen_to_merge=["午餐食咩㗎"],
        yuewen_merged="午餐食咩㗎？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="晚餐又吃什么？",
        yuewen_to_merge=["噉晚餐又食啲咩呀"],
        yuewen_merged="噉晚餐又食啲咩呀？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="那么，两客常餐吧",
        yuewen_to_merge=["噉呀", "我要两个常餐啦"],
        yuewen_merged="噉呀，我要两个常餐啦",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="今天常餐精采呀！",
        yuewen_to_merge=["好嘢呀", "我哋今日啲常餐"],
        yuewen_merged="好嘢呀，我哋今日啲常餐！",
        difficulty=2,
        verified=True,
    ),
]  # merging_test_cases_block_69
# noinspection PyArgumentList
merging_test_cases_block_70 = [
    MergingTestCase(
        zhongwen="对不起，常餐卖光了",
        yuewen_to_merge=["唔好意思", "上餐卖晒"],
        yuewen_merged="唔好意思，上餐卖晒",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="特餐？特餐有什么吃？",
        yuewen_to_merge=["特餐", "特餐有咩食㗎"],
        yuewen_merged="特餐？特餐有咩食㗎？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="午餐又吃什么呢？",
        yuewen_to_merge=["午餐食乜嘢㗎"],
        yuewen_merged="午餐食乜嘢㗎？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="什么是晚餐？",
        yuewen_to_merge=["咁乜嘢系晚餐呀"],
        yuewen_merged="咁乜嘢系晚餐呀？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="快餐吃什么？",
        yuewen_to_merge=["咁快餐食咩㗎"],
        yuewen_merged="咁快餐食咩㗎？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="唉，快餐不就是常餐",
        yuewen_to_merge=["系", "快餐就即系上餐啰"],
        yuewen_merged="系，快餐就即系上餐啰",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="常餐不是卖光了吗？",
        yuewen_to_merge=["咁你头先又话冇上餐"],
        yuewen_merged="咁你头先又话冇上餐？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="对，常餐卖光了，要吃特餐吗？",
        yuewen_to_merge=["系呀", "上餐就系卖晒呀", "咁你试唔试下特餐啦"],
        yuewen_merged="系呀，上餐就系卖晒呀，咁你试唔试下特餐啦？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="对不起，特餐卖光了",
        yuewen_to_merge=["唔好意思", "特餐卖晒嘅"],
        yuewen_merged="唔好意思，特餐卖晒嘅",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="妈妈，改快餐吧",
        yuewen_to_merge=["妈妈", "不如改快餐啦"],
        yuewen_merged="妈妈，不如改快餐啦",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="快餐有什么？",
        yuewen_to_merge=["快餐有咩㗎"],
        yuewen_merged="快餐有咩㗎？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="常餐又有什么呢？",
        yuewen_to_merge=["咁上餐有咩㗎"],
        yuewen_merged="咁上餐有咩㗎？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="那么午餐又有什么吃？",
        yuewen_to_merge=["哎呀", "咁午餐有咩食呀"],
        yuewen_merged="哎呀，咁午餐有咩食呀？",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="晚餐呢？",
        yuewen_to_merge=["咁晚餐呢"],
        yuewen_merged="咁晚餐呢？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="不是说特餐卖光了吗？",
        yuewen_to_merge=["咁你头先又话冇特餐"],
        yuewen_merged="咁你头先又话冇特餐？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="特餐卖光了，要试试快餐吗？都一样的",
        yuewen_to_merge=["系呀", "特餐系卖晒呀", "咁你试唔试下个快餐啦", "一样嘅啫"],
        yuewen_merged="系呀，特餐系卖晒呀，咁你试唔试下个快餐啦？一样嘅啫",
        difficulty=2,
        verified=True,
    ),
]  # merging_test_cases_block_70
# noinspection PyArgumentList
merging_test_cases_block_71 = [
    MergingTestCase(
        zhongwen="对不起，没快餐了",
        yuewen_to_merge=["唔好意思冇快餐呀"],
        yuewen_merged="唔好意思，冇快餐呀",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="太过份了吧？你们究竟有吃的没？",
        yuewen_to_merge=["嚟唔嚟普啲呀", "噉你哋究竟有啲咩餐呀"],
        yuewen_merged="嚟唔嚟普啲呀？噉你哋究竟有啲咩餐呀？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="午餐吧，午餐精采呀",
        yuewen_to_merge=["午餐啦", "午餐好嘢呀"],
        yuewen_merged="午餐啦，午餐好嘢呀",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="怎么个精采法？",
        yuewen_to_merge=["点好嘢法呀"],
        yuewen_merged="点好嘢法呀？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="晚餐又怎样呢？",
        yuewen_to_merge=["噉晚餐又点好嘢法呀"],
        yuewen_merged="噉晚餐又点好嘢法呀？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="常餐又怎样呢？",
        yuewen_to_merge=["噉上餐又点好嘢法呀"],
        yuewen_merged="噉上餐又点好嘢法呀？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="常餐早卖光了，你说精采不？",
        yuewen_to_merge=["上餐", "上餐一早卖晒啦", "你话好唔好嘢"],
        yuewen_merged="上餐，上餐一早卖晒啦，你话好唔好嘢？",
        difficulty=2,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="好吧好吧！两份午餐好了",
        yuewen_to_merge=["好啦好啦", "要两份午餐啦"],
        yuewen_merged="好啦好啦！要两份午餐啦",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_71
# noinspection PyArgumentList
merging_test_cases_block_72 = [
    MergingTestCase(
        zhongwen="对不起，午餐卖光了",
        yuewen_to_merge=["唔好意思", "午餐卖晒"],
        yuewen_merged="唔好意思，午餐卖晒",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="要试试我们的晚餐吗？都一样的",
        yuewen_to_merge=["试唔试下我哋嘅晚餐啦", "一样嘅啫"],
        yuewen_merged="试唔试下我哋嘅晚餐啦？一样嘅啫",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="光天白日，吃什么鬼晚餐？",
        yuewen_to_merge=["日光日白食乜鬼嘢晚餐啊"],
        yuewen_merged="日光日白，食乜鬼嘢晚餐啊？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="唉，说是说晚餐，还不就是午餐？",
        yuewen_to_merge=["系", "个名叫晚餐啫", "其实唔系真系午餐"],
        yuewen_merged="系，个名叫晚餐啫，其实唔系真系午餐？",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="好吧好吧，拜托！两份晚餐！快！",
        yuewen_to_merge=["好啦好啦", "怕咗你啦", "要两份晚餐啦", "快啲手啊"],
        yuewen_merged="好啦好啦，怕咗你啦！要两份晚餐啦！快啲手啊！",
        difficulty=1,
        verified=True,
    ),
    MergingTestCase(
        zhongwen="要快吗？那得吃快餐了！",
        yuewen_to_merge=["想快", "想快就要快餐啊"],
        yuewen_merged="想快？想快就要快餐啊！",
        difficulty=1,
        verified=True,
    ),
]  # merging_test_cases_block_72
mlamd_merge_test_cases: list[MergingTestCase] = list(
    chain.from_iterable(
        cases
        for name, cases in globals().items()
        if name.startswith("merging_test_cases_block_") and cases
    )
)
"""MLAMD 粤文 merging test cases."""

__all__ = [
    "mlamd_merge_test_cases",
]
