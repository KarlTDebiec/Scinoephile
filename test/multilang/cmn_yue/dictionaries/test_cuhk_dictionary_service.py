...
def test_scraper_default_paths_split_cache_and_database(
    tmp_path: Path,
...
def test_parse_word_file_applies_tone_mapping(tmp_path: Path):
    ...
    html_path = scraper.scraped_dir_path / "測試.html"
...
<span id="MainContent_repeaterRecord_repeaterTranslation_0_lblTranslation_0">
    ...
    entry = scraper.parser.parse_word_file(html_path)
...
def test_parse_word_file_rejects_mismatched_jyutping(tmp_path: Path):
    ...
    html_path = scraper.scraped_dir_path / "失配.html"
    html_path.parent.mkdir(parents=True, exist_ok=True)
    html_path.write_text(
        """
        <html><body>
    ...
    assert scraper.parser.parse_word_file(html_path) is None
...
def test_parse_word_file_keeps_entry_on_filename_mismatch(tmp_path: Path):
    ...
    html_path = scraper.scraped_dir_path / "測_試.html"
    html_path.parent.mkdir(parents=True, exist_ok=True)
    html_path.write_text(
        """
        <html><body>
...
def test_discover_word_links_filters_external_category_links(
    ...
    links = scraper.links.discover_word_links()
...
def test_scrape_force_clears_stale_scraped_pages(
    ...
    stale_path = scraper.scraped_dir_path / "舊詞.html"
    ...
    entries = scraper.parser.parse_scraped_pages()
