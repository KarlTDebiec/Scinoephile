...
"""Audit Zho reviews and final simplified-Hant comparison in one table."""
...
_SERIES_LABELS = {
    "hans_original": "zho-Hans source",
    "hans_review": "zho-Hans review",
    "hans_final": "zho-Hans final",
    "hant_original": "zho-Hant source",
    "hant_review": "zho-Hant review",
    "hant_final": "zho-Hant final simplified review",
}


@dataclass(frozen=True)
class AuditPaths:
    """Paths and validation rules for one Zho dual-review audit."""

    hans_original_path: Path
    """Zho-Hans review input path."""

    hans_review_path: Path
    """Zho-Hans review output path."""

    hans_final_path: Path
    """Final zho-Hans comparison path."""

    hant_original_path: Path
    """Zho-Hant review input path."""

    hant_review_path: Path
    """Zho-Hant review output path."""

    hant_final_path: Path
    """Final simplified zho-Hant comparison path."""
...
def audit_dataset(
    ...
    *,
    ...
    """Audit one Zho OCR dataset and return a Markdown report.
    ...
    paths = _get_audit_paths(dataset_name, data_root_path)
    series_paths = {
...
def format_markdown(
    *,
    ...
    Arguments:
        ...
        hans_changes: zho-Hans review changes by subtitle number
        hant_changes: zho-Hant review changes by subtitle number
    ...
    lines = [
        f"# {dataset_name} Zho Review Dual",
        "",
        "## Summary",
        "",
        f"- zho-Hans review edits: {len(hans_changes)}",
        f"- zho-Hant review edits: {len(hant_changes)}",
        ...
        *context_lines,
        ...
        "## Audit Table",
        "",
        "| Subtitle | zho-Hans | zho-Hant | zho-Hans vs Hant->Hans | Notes |",
        "|---:|---|---|---|---|",
        *row_lines,
    ]
    return "\n".join(lines) + "\n"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Audit Zho OCR review changes and final simplified-Hant differences."
        )
    )
    parser.add_argument(
        "--dataset",
        required=True,
        help="Dataset name under test/data, such as acoptc or tmm",
    )
    parser.add_argument(
        "--data-root",
...
def _get_audit_paths(
    ...
    """Get input/output paths for one OCR audit.
    ...
    hans_dir_path = output_dir_path / "zho-Hans_ocr"
    hant_dir_path = output_dir_path / "zho-Hant_ocr"
    required_dir_paths = (hans_dir_path, hant_dir_path)
    missing_dir_paths = [
        dir_path for dir_path in required_dir_paths if not dir_path.is_dir()
    ]
    if missing_dir_paths:
        missing = ", ".join(dir_path.as_posix() for dir_path in missing_dir_paths)
        raise ValueError(f"Missing Zho OCR output directory: {missing}")
    ...
    return AuditPaths(
        ...
        timing_groups=(
            ("zho-Hans source", "zho-Hans review", "zho-Hans final"),
            (
                "zho-Hant source",
                "zho-Hant review",
                "zho-Hant final simplified review",
            ),
        ),
    )
