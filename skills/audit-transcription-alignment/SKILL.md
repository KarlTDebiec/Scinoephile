...
UV_CACHE_DIR=/tmp/uv-cache uv run scinoephile audit transcription-alignment \
  ...
  --reference <independent-reference.srt> \
...
two range types are mutually exclusive. Use `--columns` to change the default
60-column stacked display width.

The displayed row order is all available ASR sources, `merged`, `speaker`, and
then `reference`. `　` is an ordinary alignment gap, `・` is a shared 250 ms pause
unit, `Ａ`/`Ｂ`/… are diarized speakers, `＊` is speech without a speaker label,
and `｜` is an evaluation-reference subtitle boundary inserted across every row.
All times are overall source times.
