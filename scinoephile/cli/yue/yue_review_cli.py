...
"""Command-line interface for 粤文 subtitle review against 中文."""
...
class YueReviewCli(CommandLineInterface):
    ...
    def _main(cls, **kwargs: Unpack[CLIKwargs]):
        ...
        yuewen_infile_path = kwargs.pop("yuewen_infile")
        zhongwen_infile_path = kwargs.pop("zhongwen_infile")
        outfile_path: Path | None = kwargs.pop("outfile")
        overwrite = kwargs.pop("overwrite")
        if yuewen_infile_path == "-" and zhongwen_infile_path == "-":
            try:
                raise ArgumentConflictError(
                    "--yuewen-infile and --zhongwen-infile may not both be '-'"
                )
        ...
        # Read inputs
        yuewen = read_series(parser, yuewen_infile_path, allow_stdin=True)
        zhongwen = read_series(parser, zhongwen_infile_path, allow_stdin=True)

        # Perform operations
        yuewen = get_yue_reviewed_vs_zho(yuewen=yuewen, zhongwen=zhongwen)
