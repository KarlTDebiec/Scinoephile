...
_YUE_HANT_PROMPT = replace(
    AlignedTranscriptionMergePrompt(
        ...
        **YUE_HANT_PROMPT_FIELDS,
        ...
        如果成個查詢都冇任何可辨識詞句得到足夠跨來源支持，`wenben` 必須回答空字串
        `""`，即係完全略過呢段。空字串係唯一唔需要「｜」結尾嘅答案。唔可以因為至少
        要輸出一項字幕而勉強揀一個來源。短片段入面，兩個來源只係偶然重合一個字，但
        各自提出唔同詞句，唔算可辨識共識。例如來源分別係「桃花」、「蒲布」、空白、
        「婆」、空白同「婆婆」時，必須回答空字串，唔可以回答「婆」或者「婆婆」。

        答案唔係字幕列表，而係一個 `wenben` 字串。輸出完整共識轉寫，並喺每項字幕結尾
