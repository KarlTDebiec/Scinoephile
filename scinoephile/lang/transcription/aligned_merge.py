...
_YUE_HANT_PROMPT = replace(
    AlignedTranscriptionMergePrompt(
        ...
        **YUE_HANT_PROMPT_FIELDS,
        ...
        具體詞句必須有跨來源嘅語音轉寫支持。如果一個詞或者連續片段只出現喺單一來源，
        而其他來源喺同一時間位置都係空格，原則上要當作單一模型插入或者幻覺而刪除，尤其
        係突然轉語言嘅英文等孤立片段。講者列只可以證明可能有人聲；語言列只係語言分類；
        歌唱同音樂列只係聲音事件。呢啲分析列全部都唔係獨立嘅詞彙證據，唔可以替單一來源
        嘅具體文字背書。只有其他來源喺相鄰或者重疊欄提供同一發音或同一語句嘅另一寫法，
        清楚顯示問題係對齊偏移而唔係孤立插入，先可以保留單一列顯示嘅形式。唔好因為孤立
        文字語法通順、符合語境或者同語言分類一致就收錄。

        答案唔係字幕列表，而係一個 `wenben` 字串。輸出加上合適全形中文標點嘅完整共識
        轉寫，並喺每項字幕結尾插入「｜」；答案最後一個字符都必須係「｜」。例如兩項字幕
        可以回答 `甲。｜乙！｜`。除咗「｜」之外，答案唔可以包含輸入嘅全形空格、停頓、
        講者或者語音分析標記，亦唔需要逐欄對齊或者補空格。刪走所有「｜」後嘅完整文本
        唔可以遺漏、重複、概括、翻譯或者虛構內容。保留實際對白嘅意思、語域、粗口、重複、
        猶豫、語氣助詞同人物口吻。採用香港繁體粵文；英文同其他拉丁文字都要用全形字母
        同數字。
        ...
        language_trace="yuyan",
        language_trace_desc="逐欄對齊嘅全形語言、未分類同停頓標記（如果有）",
        singing_trace="gechang",
        singing_trace_desc="逐欄對齊嘅歌唱、冇歌唱同停頓標記（如果有）",
        music_trace="yinyue",
        music_trace_desc="逐欄對齊嘅音樂、冇音樂同停頓標記（如果有）",
        answer_text="wenben",
        answer_text_desc="加上合適標點並喺每項字幕後插入全形直線嘅完整共識轉寫",
        ...
        audio_event_character_err=(
            "歌唱同音樂列只可以包含相應嘅全形事件、全形空格同停頓標記。"
        ),
        transcript_empty_err="對齊轉寫合併查詢必須包含轉寫文本。",
        answer_text_err=(
            "答案文本必須包含非空白字幕，以全形直線分隔並結束，而且唔可以包含對齊或者"
            "講者標記。"
        ),
