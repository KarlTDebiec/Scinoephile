...
class Aligner:
    ...
    async def _shift(self, alignment) -> bool:
        ...
        for sg_1_idx in range(len(alignment.sync_groups) - 1):
            ...
            yuewen_1_shifted = getattr(
                answer, test_case.prompt_cls.src_2_sub_1_shifted_field, None
            )
            yuewen_2_shifted = getattr(
                answer, test_case.prompt_cls.src_2_sub_2_shifted_field, None
            )
    ...
    def _shift_one(
        ...
        yw_1_idxs = sg_1[1]
        yw_2_idxs = sg_2[1]
        yw_1 = getattr(query, query.prompt_cls.src_2_sub_1_field, None)
        yw_2 = getattr(query, query.prompt_cls.src_2_sub_2_field, None)
        yw_1_shifted = getattr(answer, query.prompt_cls.src_2_sub_1_shifted_field, None)
        yw_2_shifted = getattr(answer, query.prompt_cls.src_2_sub_2_shifted_field, None)
