...
"""Code related to English guided translation from written Cantonese."""
...
from .prompts import EngGuidedTranslationVsYuePrompt

__all__ = [
    "ENG_YUE_GUIDED_TRANSLATION_OPERATION_SPEC",
    "EngGuidedTranslationVsYuePrompt",
    "EngYueGuidedTranslationProcessKwargs",
    "EngYueGuidedTranslationProcessorKwargs",
    "get_eng_translated_from_yue_with_eng_guidance",
    "get_eng_yue_guided_translator",
]

ENG_YUE_GUIDED_TRANSLATION_OPERATION_SPEC = OperationSpec(
    operation="eng-yue-guided-translation",
    test_case_table_name="test_cases__eng_yue__guided_translation",
    manager_cls=DualNToMManager,
    prompt_cls=EngGuidedTranslationVsYuePrompt,
)
"""Operation specification for English guided translation from written Cantonese."""


class EngYueGuidedTranslationProcessKwargs(TypedDict, total=False):
...
class EngYueGuidedTranslationProcessorKwargs(TypedDict, total=False):
...
def get_eng_translated_from_yue_with_eng_guidance(
    yuewen: Series,
    eng: Series,
    translator: DualNToMProcessor | None = None,
    **kwargs: Unpack[EngYueGuidedTranslationProcessKwargs],
) -> Series:
    """Get English subtitles guided-translated from written Cantonese.
    ...
    Returns:
        English subtitles guided-translated from written Cantonese
    """
    if translator is None:
        translator = get_eng_yue_guided_translator()
    return translator.process(yuewen, eng, **kwargs)


def get_eng_yue_guided_translator(
    prompt_cls: type[EngGuidedTranslationVsYuePrompt] = (
        EngGuidedTranslationVsYuePrompt
    ),
    test_cases: list[TestCase] | None = None,
    provider: LLMProvider | None = None,
    **kwargs: Unpack[EngYueGuidedTranslationProcessorKwargs],
) -> DualNToMProcessor:
