...
class FusionFuser:
    ...
    def __init__(
        ...
        default_test_cases: Sequence[FusionTestCase] | None = None,
    ):
        """Initialize.
        ...
        test_case_list: list[FusionTestCase]
        if test_cases is None:
            if default_test_cases is not None:
                test_case_list = list(default_test_cases)
            else:
                test_case_list = []
        else:
    ...
    def fuse(
        ...
        for sub_idx, (sub_one, sub_two) in enumerate(zip(source_one, source_two)):
            ...
            if not text_two:
                output_subtitles.append(sub_one)
                info(
                    f"Subtitle {sub_idx + 1} from {self.prompt_cls.source_one_label}: "
                    f"{text_one.replace('\n', ' ')}"
                )
                continue
            if not text_one:
                ...
                info(
                    f"Subtitle {sub_idx + 1} from {self.prompt_cls.source_two_label}: "
                    f"{text_two.replace('\n', ' ')}"
                )
                continue

            # Query LLM
            test_case_cls = FusionTestCase.get_test_case_cls(prompt_cls=self.prompt_cls)
    ...
    def get_default_test_cases(cls):
        ...
        Returns:
            test cases
        """
        warning(f"No default test cases defined for base class {cls.__name__}.")
        return []
