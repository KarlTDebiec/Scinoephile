...
"""Factories for block review LLM classes."""
...
__all__ = ["BlockReviewManager"]


class BlockReviewManager(Manager):
    """Factories for block review LLM classes."""

    prompt_cls: ClassVar[type[BlockReviewPrompt]] = BlockReviewPrompt
    ...
    def get_answer_cls(
        cls,
        size: int,
        prompt_cls: type[BlockReviewPrompt] = BlockReviewPrompt,
    ) -> type[Answer]:
        ...
        Returns:
            answer model class
        """
        name = get_model_name("BlockReviewAnswer", f"{size}_{prompt_cls.__name__}")
        fields: dict[str, Any] = {}
    ...
    def get_query_cls(
        ...
        Returns:
            query model class
        """
        name = get_model_name("BlockReviewQuery", f"{size}_{prompt_cls.__name__}")
        fields: dict[str, Any] = {}
        for idx in range(size):
    ...
    def get_test_case_cls(
        ...
        """
        name = get_model_name("BlockReviewTestCase", f"{size}_{prompt_cls.__name__}")
    ...
    def get_test_case_cls_from_data(
        ...
        **kwargs: Unpack[TestCaseClsKwargs],
        ...
        prompt_cls = cast(type[BlockReviewPrompt], prompt_cls)
    ...
    def get_min_difficulty(model: TestCase) -> int:
        ...
        Returns:
            minimum difficulty
        """
        prompt_cls: type[BlockReviewPrompt] = getattr(model, "prompt_cls")
