...
class SubtitleSeries(Base, SSAFile):  # type: ignore
    ...
    Extension of pysubs2's SSAFile with additional features. Includes
    code for loading to and saving from hdf5. While these are part of
    separate classes in pysubs.SSAFile, this separation is not really
    beneficial here.
    """
    ...
    def __str__(self) -> str:
        ...
        if self.events:
            ...
            return f"<SubtitleSeries with 0 events " f"and {len(self.styles):d} styles>"
    ...
    def save(self, path: str, format_: Optional[str] = None, **kwargs: Any) -> None:
        """
        ...
        if self.verbosity >= 1:
            print(f"Writing subtitles to '{path}'", file=stderr)
    ...
    def load(
        ...
        if verbosity >= 1:
            print(f"Reading subtitles from '{path}'", file=stderr)
        ...
        # Otherwise, use SSAFile.from_file
            ...
            for ssaevent in subs.events:
                events.append(
                    cls.event_class(
                        verbosity=verbosity, series=subs, **ssaevent.as_dict()
                    )
                )
            subs.events = events
    ...
    def _save_hdf5(self, fp: Any, **kwargs: Any) -> None:
        ...
        def style_value_to_string(field: str, style_: SSAStyle) -> str:
            ...
            if field in {"start", "end"}:
                ...
                raise TypeError(
                    f"Unexpected type when writing a SubStation "
        ...
        fp.create_dataset(
            "styles", data=styles, dtype=dtypes, chunks=True, compression="gzip"
        )
        ...
        fp.create_dataset(
            "events", data=events, dtype=dtypes, chunks=True, compression="gzip"
        )
    ...
    def _load_hdf5(
        ...
        if "styles" in fp:
            ...
            for style in styles:
                style = {
                    field.lower(): style_value_from_string(field, value.decode("utf8"))
        ...
        if "events" in fp:
            ...
            for event in events:
                ...
                subs.events.append(
                    cls.event_class(verbosity=verbosity, series=subs, **event)
                )
